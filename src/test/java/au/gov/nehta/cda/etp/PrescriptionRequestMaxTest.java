...
//        assertTrue(check.schematronErrors.size() == 0);
    ...
    public void generateMax()
        ...
        ZonedDateTime dateOfBirth = LocalDate.of(1982, 11, 28).atStartOfDay(ZoneId.systemDefault());
        ...
        SubjectOfCareParticipant subjectOfCareParticipant = new SubjectOfCareParticipantImpl(Collections.singletonList(ihi), subjectOfCareAddress, subjectOfCarePerson);
...
//		SubjectOfCareParticipation subjectOfCareParticipation = new SubjectOfCareParticipationImpl( subjectOfCareParticipationType, subjectOfCareParticipant );
        ...
        dispensingOrganisationEntitlement.setEntitlementValidityDuration(
                RestrictedTimeInterval.getLowInstance(
                        // From today (day only precision)
                        new PrecisionDate(Precision.DAY, ZonedDateTime.now())));
        dispensingOrganisationParticipant.setEntitlement(Collections.singletonList(dispensingOrganisationEntitlement));
...
//		Occupation prescriberOccupation = ParticipationFactory.createOccupation( ANZSCO_1ED_2006.GENERAL_MEDICAL_PRACTITIONER );
        ...
        List<Telecom> prescriberComms = new ArrayList<>();
        ...
        ZonedDateTime dateTimePrescriptionRequestWritten = ZonedDateTime.now();
        ...
        ZonedDateTime recipientDateOfBirth = ZonedDateTime.of(1982, 11, 28, 0, 0, 0, 0, ZoneId.systemDefault());
        // Code therapeuticGood = new SNOMED_AU_Code( "1.1.1.1.1","therapeutic good code display name" );
        ...
        AustralianAddress prescriberInstructionAddress;
        prescriberInstructionAddress = prescriberAustralianAddress;// reuse the prescriber's address
        ...
        prescriberInstruction.setElectronicCommunicationDetail(Collections.singletonList(prescriberInstructionRecipientElectronicCommunicationDetail));
        ...
        ZonedDateTime dateTimeInstructionReceived = ZonedDateTime.now();
...
//        intervalBetweenRepeats.setNarrative( "3 weeks" ); // a text description of this time quantity
        ...
        ZonedDateTime twoWeeks = ZonedDateTime.now();
        ...
        String additionalComments = "Patient must return for a checkup at the end of treatment";
        ...
        SNOMED_AU_Code dispensingUnitCode = new SNOMED_AU_Code("32570641000036102", "foo bar dispensing unit");
...
//        onceADay.setNarrative( "Once every twenty four (24) Hours" );
        ...
        */
        ...
        ZonedDateTime prescriberInstructionTime = ZonedDateTime.of(2009, 10, 20, 12, 35, 0, 0, ZoneId.systemDefault());
        ...
        authenticatorAssignedEntity.setAddress(Collections.singletonList(authenticatorPostalAddress));

        Telecom authenticatorEmail = new TelecomImpl(TelecomMedium.EMAIL, "sample@example.com.au", TelecomUse.BUSINESS);
        authenticatorAssignedEntity.setTelecom(Collections.singletonList(authenticatorEmail));

        LegalAuthenticator cdaLegalAuthenticator = LegalAuthenticatorImpl.getInstance(ZonedDateTime.now(), authenticatorAssignedEntity);
        ...
        CustodianOrganization custodianOrganization = CustodianOrganizationImpl.getInstance(Collections.singletonList(id), custodianHPIO);
