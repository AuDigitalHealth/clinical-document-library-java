...
public final class IHI extends AsEntityIdentifierImpl implements AsEntityIdentifier {
	...
	public IHI( String ihiNumber ) {
		...
		if (ihiNumber.length() != 16) {
			throw new IllegalArgumentException( "the ihiNumber parameter needs to a be 16 character string" );
		}

		try {
			Long.parseLong( ihiNumber );
		} catch (NumberFormatException exception) {
			throw new IllegalArgumentException( "the ihiNumber parameter needs to be a 16 digit number" );
		}
