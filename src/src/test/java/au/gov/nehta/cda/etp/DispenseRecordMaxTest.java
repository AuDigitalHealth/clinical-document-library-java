...
public class DispenseRecordMaxTest extends Base{
    ...
    private static final String SCHEMATRON_TEMPLATE_PATH =      "resources/dispenseRecord";
    

    
    private static final String DOCUMENT_FILE_NAME = TEST_GENERATION+"dispense-max.xml";
    
    
    @Test @Ignore
    public void test_MAX_DispenseRecordCreation() throws ParserConfigurationException, JAXBException, SchemaValidationException, SchematronValidationException, IOException {
        generateMax();
	...
	public void generateMax() throws ParserConfigurationException, JAXBException, SchematronValidationException {
		// Setup context
		// Subject Of Care
        ...
        */
		...
		SubjectOfCareParticipant subjectOfCareParticipant =new SubjectOfCareParticipantImpl( Arrays.asList( ihi ), Arrays.asList(subjectOfCareAddress), subjectOfCarePerson );
        ...
        ProviderAddress dispenserProviderAddress = new ProviderAddressImpl( false, dispenserAddress, AddressPurpose.BUSINESS );
        
        
        DispenserParticipant dispenserParticipant = new DispenserParticipantImpl( dispenserHPII, PersonNameImpl.getInstance( dispenserFamilyName,"Duncan",NameTitle.DOCTOR.getDescriptor(), PersonNameUsage.REGISTERED_NAME_OR_LEGAL_NAME ),dispenserProviderAddress );
		...
		dispensingOrgAddress.addUnstructuredAddressLine( "London Drugs, Shop 42, Level 2, 10 Browning street" );
		...
		DispensingOrganisationParticipant dispensingOrganisationParticipant = new DispensingOrganisationParticipantImpl( dispensingOrgHPIO,
				dispensingOrganisationAddress, electronicCommunicationDetailList, dispensingOrganisation );

		AsEntityIdentifier dispensingOrgEntitlement = new AsEntityIdentifierImpl("1.22.333.4444","123456781");
		...
		dispensingOrganisationEntitlement.setEntitlementValidityDuration(
				RestrictedTimeInterval.getLowHighInstance( new PrecisionDate(Precision.DAY, 1l), new PrecisionDate(Precision.DAY) )
				);
		...
		DispenseRecordContext dispenseRecordContext = new DispenseRecordContextImpl( subjectOfCareParticipant, dispenserParticipation, dispensingOrganisationParticipation );
        ...
        QuantityUnitDescription dispensedQuantity = QuantityUnitDescriptionImpl.create( 7, dispensingUnitCode );
		...
		DispenseItem dispenseItem = builder.build(); //throws illegal argument exception if missing mandatory fields
		...
		PersonName authenticatorName = PersonNameImpl.getInstance( "Black", "Jane", NameTitle.MRS.getDescriptor(), NameSuffix.JUNIOR.getDescriptor(), PersonNameUsage.REGISTERED_NAME_OR_LEGAL_NAME );
		...
		LegalAuthenticator cdaLegalAuthenticator = LegalAuthenticatorImpl.getInstance( new DateTime(), authenticator );
	    ...
	    PostalAddress authenticatorPostal1 = new PostalAddressImpl(authenticatorAddress1,PostalAddressUseEnum.POSTAL_ADDRESS);
        ...
        PostalAddress authenticatorPostal2= new PostalAddressImpl(authenticatorAddress1,PostalAddressUseEnum.TEMPORARY_ADDRESS);
		...
		PostalAddress pa = new PostalAddressImpl(custodianPostalAddress, PostalAddressUseEnum.WORKPLACE);

		UniqueIdentifier uniqueId = UniqueIdentifierImpl.random();
		UniqueIdentifier otherId = UniqueIdentifierImpl.random();
		
		
		CustodianOrganization custodianOrg = CustodianOrganizationImpl.getInstance( Arrays.asList(uniqueId,otherId),custodianIdentifier );
