...
<xsl:stylesheet xmlns:cda="urn:hl7-org:v3"
  ...
  <xsl:output doctype-public="-//W3C//DTD HTML 4.01//EN"
    doctype-system="http://www.w3.org/TR/html4/strict.dtd" encoding="UTF-8" indent="yes"
    method="html"
    version="4.01"/>
        ...
        <xsl:element name="div">
            ...
            <xsl:choose>
              <xsl:otherwise>
                ...
                <xsl:call-template name="processStructuredBody">
                  <xsl:with-param name="structuredBody"
                    select="cda:component/cda:structuredBody"/>
                  <xsl:with-param name="processAdministrativeObservations">no
                  </xsl:with-param>
                </xsl:call-template>
                <xsl:if test="cda:component/cda:structuredBody/cda:component/cda:section[(cda:code/@code='102.16080') and (cda:code/@codeSystem='1.2.36.1.2001.1001.101')] and
                  ...
                  <xsl:call-template name="processStructuredBody">
                    <xsl:with-param name="structuredBody"
                      select="cda:component/cda:structuredBody"/>
                    <xsl:with-param name="processAdministrativeObservations">
                      yes
                    </xsl:with-param>
                  </xsl:call-template>
                </xsl:if>
          ...
          <xsl:element name="div">
            ...
            <xsl:choose>
              <xsl:otherwise/>
              <xsl:when
                test="$cdaDocumentType=$DH_DISCHARGE_SUMMARY_CLINICAL_DOCUMENT_TYPE_NAME">
          ...
          <xsl:if
            test="$isStylesheetCompatibleWithRequiredRenderingSpecificationVersion='false'">
            <xsl:call-template name="getRenderingSpecificationIncompatibilityWarning"/>
          </xsl:if>

          <!-- Display footnotes -->
          <xsl:if
            test="count(./cda:participant[(@typeCode='PART' or @typeCode='IRCP') and cda:associatedEntity/cda:associatedPerson]) &gt; 0">
            <xsl:call-template name="getParticipantsTable">
              ...
              <xsl:with-param name="participants"
                select="./cda:participant[(@typeCode='PART' or @typeCode='IRCP') and cda:associatedEntity/cda:associatedPerson]"/>
            </xsl:call-template>
          </xsl:if>

          <!-- Display End of Document -->
          <xsl:if
            test="count(./cda:informationRecipient[@typeCode='PRCP' or not(@typeCode)]/cda:intendedRecipient) &gt; 0">
            <xsl:call-template name="getRecipientsTable">
              ...
              <xsl:with-param name="recipients"
                select="./cda:informationRecipient[@typeCode='PRCP' or not(@typeCode)]/cda:intendedRecipient"/>
            </xsl:call-template>
          </xsl:if>
          <xsl:if
            test="count(./cda:informationRecipient[@typeCode='TRC']/cda:intendedRecipient) &gt; 0">
            <xsl:call-template name="getRecipientsTable">
              <xsl:with-param name="title">Secondary Recipients</xsl:with-param>
              <xsl:with-param name="tableId">SECONDARY_RECIPIENTS_TABLE
              </xsl:with-param>
              <xsl:with-param name="recipients"
  ...
  <xsl:template name="getPDFinIFrame">
    <xsl:if test="$supportiFrameForCertainDocTypes!='false'">
      ...
      <xsl:if test="($cdaDocumentType=$DH_PATHOLOGY_REPORT_CLINICAL_DOCUMENT_TYPE_NAME
        ...
        <xsl:if
          test="string-length(//cda:reference/cda:externalDocument/cda:text/cda:reference/@value) &gt; 0">
          <xsl:element name="div">
            <xsl:attribute name="style">margin-left:<xsl:value-of
              select="$LEFT_MARGIN_WIDTH_PX"/>px
            </xsl:attribute>
            <xsl:element name="iframe">
              ...
              <xsl:attribute name="width">
                <xsl:value-of select="$IFRAME_CONTENT_WIDTH_PX"/>
              </xsl:attribute>
              <xsl:attribute name="height">
                <xsl:value-of select="$IFRAME_CONTENT_HEIGHT_PX"/>
              </xsl:attribute>
            </xsl:element>
          </xsl:element>
        </xsl:if>
      ...
      <xsl:if
        test="($cdaDocumentType=$DH_PHARMACIST_SHARED_MEDICINES_LIST_CLINICAL_DOCUMENT_TYPE_NAME)">
        <!-- Is there an external document reference? -->
        <xsl:if
          test="string-length(//cda:entry/cda:observationMedia[@ID!='LOGO']/cda:value/cda:reference/@value) &gt; 0">
          <xsl:element name="div">
            <xsl:attribute name="style">margin-left:<xsl:value-of
              select="$LEFT_MARGIN_WIDTH_PX"/>px
            </xsl:attribute>
            <xsl:element name="iframe">
              <xsl:attribute name="src">
                <xsl:value-of
                  select="//cda:entry/cda:observationMedia[@ID!='LOGO']/cda:value/cda:reference/@value"/>
              </xsl:attribute>
              <xsl:attribute name="scrolling">yes</xsl:attribute>
              <xsl:attribute name="frameborder">1</xsl:attribute>
              <xsl:attribute name="width">
  ...
  <xsl:template name="getBanner">
    <xsl:element name="div">
      ...
      <xsl:if test="$completionCode = 'Withdrawn'">
        <xsl:value-of select="concat(' – ', 'WITHDRAWN')"/>
      </xsl:if>
    ...
    <xsl:element name="div">
      ...
      <xsl:choose>
        <xsl:otherwise>
          <xsl:value-of select="$cdaDocumentCreationDate"/>
        </xsl:otherwise>
        <xsl:when
          test="$cdaDocumentType=$DH_DISCHARGE_SUMMARY_CLINICAL_DOCUMENT_TYPE_NAME and (string-length($encounterEndDate) &gt; 0)">
          <xsl:value-of select="$encounterEndDate"/>
        </xsl:when>
      </xsl:choose>
    </xsl:element> <!-- </div> -->
    ...
    <xsl:element name="div">
        ...
        <xsl:choose>
          ...
          <xsl:when
            ...
            <xsl:if
              ...
              <xsl:text> </xsl:text><xsl:value-of select="$patientGivenName"/>
            </xsl:if>
            <xsl:if test="$patientPrefix!='' and string-length($patientPrefix) &gt; 0">
              <xsl:text> </xsl:text><xsl:value-of select="$patientPrefix"/>
            </xsl:if>
          </xsl:when>
        </xsl:choose>
      </xsl:element>  <!-- </div> -->
      ...
      <xsl:element name="dl">
        ...
        <xsl:element name="dd">
          ...
          <xsl:choose>
            <xsl:otherwise>
              <xsl:value-of select="$patientDateOfBirth"/><xsl:value-of
              select="concat(' (', $patientAge, ')')"/>
            </xsl:otherwise>
            <xsl:when
              test="not($patientAge='DECEASED') and not($patientAge='AGE UNKNOWN') and string-length($patientCodedAge) = 0">
    ...
    <xsl:variable name="completionCode">
      <xsl:if test="/cda:ClinicalDocument/ext:completionCode and
        ...
        <xsl:choose>
          <xsl:otherwise/>
          <xsl:when test="/cda:ClinicalDocument/ext:completionCode/@code='I'">Interim
          </xsl:when>
          <xsl:when test="/cda:ClinicalDocument/ext:completionCode/@code='W'">Withdrawn
          </xsl:when>
          <xsl:when test="/cda:ClinicalDocument/ext:completionCode/@code='F'">Final
          </xsl:when>
        </xsl:choose>
      </xsl:if>
    </xsl:variable> <!-- </dl> -->
  ...
  <xsl:template name="getAuthorDetailsAndLogoTable">
    <xsl:if test="string-length($authorOrganisation) &gt; 0 or
      ...
      <xsl:element name="table">
        ...
        <xsl:element name="tr">
          <xsl:element name="td">
            <xsl:attribute name="class">documentTitleTableAuthorDetailsTd
            </xsl:attribute>
            <xsl:element name="div">
            ...
            <xsl:if test="string-length($authorName) &gt; 0">
              <xsl:element name="div">
                <xsl:element name="table">
                  <xsl:attribute name="class">documentTitleAuthorDetailsTable
                  </xsl:attribute>
                  <xsl:element name="caption"><xsl:attribute name="class">
                    sr-only-pdf</xsl:attribute>Document Author
                  </xsl:element>
                  <xsl:element name="tbody">
                    <xsl:choose>
                      ...
                      <xsl:when
                        ...
                        <xsl:if
                          test="string-length($dischargeDispositionCode/@displayName) &gt; 0 or string-length($dischargeDispositionCode/cda:originalText) &gt; 0 or string-length($dischargeDispositionCode/@nullFlavor) &gt; 0">
                          <xsl:element name="tr">
                            <xsl:element name="th">
                            ...
                            <xsl:element name="td">
                              ...
                              <xsl:choose>
                                <xsl:when
                                  test="string-length($dischargeDispositionCode/cda:originalText) &gt; 0">
                                  <xsl:value-of
                                    select="$dischargeDispositionCode/cda:originalText"/>
                                </xsl:when>
                                <xsl:when
                                  test="string-length($dischargeDispositionCode/@displayName) &gt; 0">
                                  <xsl:value-of
                                    select="$dischargeDispositionCode/@displayName"/>
                                </xsl:when>
                                <xsl:when
                                  test="string-length($dischargeDispositionCode/@nullFlavor) &gt; 0">
                                  <xsl:call-template
                                    name="show-null-flavor-value">
                                    <xsl:with-param
                                      name="nullFlavorValue"
                                      select="$dischargeDispositionCode/@nullFlavor"></xsl:with-param>
                                  </xsl:call-template>
                                </xsl:when>
                              </xsl:choose>
                        ...
                        <xsl:if
                          test="string-length($dischargeLocation) &gt; 0">
                          <xsl:element name="tr">
                            <xsl:element name="th">
                              <xsl:attribute name="class">
                                documentTitleAuthorDetailsTableTh
                        ...
                        <xsl:if test="string-length($lengthOfStay) &gt; 0">
                          <xsl:element name="tr">
                            ...
                            <xsl:element name="td">
                              ...
                              <xsl:value-of select="$lengthOfStay"/>
                              day/s
                            </xsl:element>
                          </xsl:element> <!-- </tr> -->
                        </xsl:if>
                        <xsl:variable name="dischargeDispositionCode"
                          select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:dischargeDispositionCode"/>

                      </xsl:when>
                      <xsl:when
                        test="$cdaDocumentType = $DH_DIAGNOSTIC_IMAGING_REPORT_CLINICAL_DOCUMENT_TYPE_NAME">
                      ...
                      <xsl:when
                        test="$cdaDocumentType = $DH_EVENT_SUMMARY_CLINICAL_DOCUMENT_TYPE_NAME">
                        ...
                        <xsl:element name="tr">
                          ...
                          <xsl:element name="td">
                            ...
                            <xsl:choose>
                              <xsl:otherwise/>
                              <xsl:when
                                test="(string-length($encounterStartDate) &gt; 0) and (string-length($encounterEndDate) &gt; 0)">
                                <xsl:value-of
                                  select="concat($encounterStartDateTime, ' to ', $encounterEndDateTime)"/>
                              </xsl:when>
                              <xsl:when
                                test="string-length($encounterEndDate) &gt; 0">
                                <xsl:value-of
                                  select="$encounterEndDateTime"/>
                              </xsl:when>
                              <xsl:when
                                test="string-length($encounterDuration) &gt; 0">
                                <xsl:value-of
                                  select="$encounterDuration"/>
                              </xsl:when>
                              <xsl:when
                                test="string-length($encounterEffectiveTimeNullFlavor) &gt; 0">
                                <xsl:value-of
                                  select="$encounterEffectiveTimeNullFlavor"/>
                              </xsl:when>
                              <xsl:when
                                test="string-length($encounterDate) &gt; 0">
                                <xsl:value-of
                      ...
                      <xsl:element name="tr">
                        <xsl:element name="th">
                          ...
                          <xsl:choose>
                            ...
                            <xsl:when
                              test="$cdaDocumentType = 'eHealth Dispense Record'">
                              Dispenser
                            </xsl:when>
                            <xsl:when
                              test="$cdaDocumentType = 'eHealth Prescription Record'">
                              Prescriber
                            </xsl:when>
                          </xsl:choose>
                        </xsl:element>
                        <xsl:element name="td">
                          ...
                          <xsl:choose>
                            ...
                            <xsl:when
                              test="string-length($authorRoleOT) &gt; 0">
                              (<xsl:value-of
                              select="$authorRoleOT"/>)
                            </xsl:when>
                          </xsl:choose>
          ...
          <xsl:element name="td">
            ...
            <xsl:if
              test="/cda:ClinicalDocument/cda:component/cda:structuredBody//cda:observationMedia[@ID='LOGO']/cda:value[@mediaType='image/png' or @mediaType='image/gif' or @mediaType='image/jpeg']">
    ...
    <xsl:variable name="authorName">
      <xsl:call-template name="show-name">
        <xsl:with-param name="name"
          select="/cda:ClinicalDocument/cda:author/cda:assignedAuthor[1]/cda:assignedPerson/cda:name[1]"/>
      </xsl:call-template>
    </xsl:variable>
    <xsl:variable name="authorRole"
      select="/cda:ClinicalDocument/cda:author/cda:assignedAuthor[1]/cda:code/@displayName"/>
    <xsl:variable name="authorRoleOT"
      select="/cda:ClinicalDocument/cda:author/cda:assignedAuthor[1]/cda:code/cda:originalText"/>
    <xsl:variable name="authorPhoneNumber"
      select="substring-after(/cda:ClinicalDocument/cda:author/cda:assignedAuthor[1]/cda:telecom[starts-with(@value, 'tel')]/@value, 'tel:')"/>
    <xsl:variable name="facilityName"
      select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:location/cda:healthCareFacility/cda:serviceProviderOrganization/cda:asOrganizationPartOf/cda:wholeOrganization/cda:name"/>
    <xsl:variable name="facilityDepartment"
      select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:location/cda:healthCareFacility/cda:serviceProviderOrganization/cda:name"/>
    ...
    <xsl:variable name="authorOrganisation">
      <xsl:choose>
        <xsl:when
          test="string-length(/cda:ClinicalDocument/cda:author/cda:assignedAuthor/cda:assignedPerson/ext:asEmployment/ext:employerOrganization/cda:asOrganizationPartOf/cda:wholeOrganization/cda:name) &gt; 0">
          <xsl:value-of
            select="/cda:ClinicalDocument/cda:author/cda:assignedAuthor/cda:assignedPerson/ext:asEmployment/ext:employerOrganization/cda:asOrganizationPartOf/cda:wholeOrganization/cda:name"/>
        </xsl:when>
        <xsl:when
          test="string-length(/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:location/cda:healthCareFacility/cda:serviceProviderOrganization/cda:asOrganizationPartOf/cda:wholeOrganization/cda:name) &gt; 0">
          <xsl:value-of
            select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:location/cda:healthCareFacility/cda:serviceProviderOrganization/cda:asOrganizationPartOf/cda:wholeOrganization/cda:name"/>
        </xsl:when>
        <xsl:when
          test="/cda:ClinicalDocument/cda:author/cda:assignedAuthor/cda:assignedAuthoringDevice">
          <xsl:value-of
            select="/cda:ClinicalDocument/cda:author/cda:assignedAuthor/cda:assignedAuthoringDevice/cda:softwareName/text()"/>
        </xsl:when>
      </xsl:choose>
    </xsl:variable>
  </xsl:template>
      ...
      <xsl:element name="table">
        ...
        <xsl:element name="tr">
          ...
          <xsl:element name="td">
            ...
            <xsl:call-template
              name="getResponsibleHealthProfessionalAtTimeOfDischargeRow"/>
          </xsl:element> <!-- </td> -->
          <xsl:element name="td">
            <xsl:attribute name="class">detailsTableTd</xsl:attribute>
  ...
  <xsl:template name="getEncounterDetails">
    <xsl:if test="$encounterStartDateTime != '' or
      ...
      <xsl:element name="table">
        ...
        <xsl:element name="tbody">
          <xsl:if
            test="string-length($encounterEffectiveTimeNullFlavor) &gt; 0 and string-length($encounterStartDateTime) = 0 and string-length($encounterEndDateTime) = 0 and string-length($encounterDateTime) = 0">
            <xsl:element name="tr">
              <xsl:element name="th">
                <xsl:attribute name="class">detailsHalfTableTh</xsl:attribute>
                <xsl:element name="span">
                  <xsl:attribute name="class">detailsHalfTableThSpan
                  </xsl:attribute>
                  Date
                </xsl:element>
              </xsl:element> <!-- </th> -->
              ...
              <xsl:element name="th">
                ...
                <xsl:element name="span">
                  ...
                  <xsl:choose>
                    ...
                    <xsl:when
                      test="$cdaDocumentType=$DH_SPECIALIST_LETTER_CLINICAL_DOCUMENT_TYPE_NAME">
                      Date/Time Subject of Care Seen
                    </xsl:when>
                  </xsl:choose>
                </xsl:element>
              </xsl:element> <!-- </th> -->
            ...
            <xsl:element name="tr">
              ...
              <xsl:element name="td">
                ...
                <xsl:element name="ul">
                  ...
                  <xsl:for-each
                    select="/cda:ClinicalDocument/cda:component/cda:structuredBody/cda:component/cda:section/cda:entry/cda:observation[cda:code/@code='103.16028' and cda:code/@codeSystem='1.2.36.1.2001.1001.101' and cda:code/@displayName='Specialty']/cda:value">
                    <xsl:element name="li">
                      <xsl:attribute name="class">specialtiesli
                      </xsl:attribute>
                      <xsl:choose>
                        <xsl:otherwise>
                          <xsl:if
                            test="./cda:originalText and string-length(./cda:originalText) &gt; 0">
                            <xsl:value-of
                              select="./cda:originalText/text()"/>
                          </xsl:if>
                        </xsl:otherwise>
  ...
  <xsl:template name="getResponsibleHealthProfessionalAtTimeOfDischargeRow">
    <xsl:if test="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:encounterParticipant[@typeCode='DIS']/cda:assignedEntity/cda:assignedPerson/cda:name and
      ...
      <xsl:element name="table">
        ...
        <xsl:element name="caption">
          <xsl:attribute name="class">sr-only-pdf</xsl:attribute>
          Administrative details for Responsible Health Professional At Time Of Discharge
        </xsl:element>
        ...
        <xsl:element name="tbody">
          <xsl:call-template name="getAddressDetailsRows">
            <xsl:with-param name="personOrOrg"
              select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:encounterParticipant[@typeCode='DIS']/cda:assignedEntity"/>
          </xsl:call-template>
          ...
          <xsl:element name="tr">
            ...
            <xsl:element name="td">
              <xsl:attribute name="class">detailsHalfTableTd</xsl:attribute>
              <xsl:for-each
                select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:encounterParticipant[@typeCode='DIS']/cda:assignedEntity/cda:assignedPerson/cda:name">
                <xsl:call-template name="show-name">
              ...
              <xsl:for-each
                select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:encounterParticipant[@typeCode='DIS']/cda:assignedEntity/cda:assignedPerson/ext:asEntityIdentifier">
            ...
            <xsl:element name="tr">
              ...
              <xsl:element name="td">
                ...
                <xsl:call-template name="getProviderOrganisationIdentifier">
                  <xsl:with-param name="entityIdentifier"
                    select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:encounterParticipant[@typeCode='DIS']/cda:assignedEntity/cda:assignedPerson/ext:asEmployment/ext:employerOrganization/cda:asOrganizationPartOf/cda:wholeOrganization/ext:asEntityIdentifier"/>
                </xsl:call-template>
                <xsl:value-of
                  select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:encounterParticipant[@typeCode='DIS']/cda:assignedEntity/cda:assignedPerson/ext:asEmployment/ext:employerOrganization/cda:asOrganizationPartOf/cda:wholeOrganization/cda:name"/>
              </xsl:element>
            </xsl:element>
          </xsl:if>
          ...
          <xsl:if test="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:encounterParticipant[@typeCode='DIS']/cda:assignedEntity/cda:assignedPerson/ext:asEmployment/ext:employerOrganization/cda:name and
            ...
            <xsl:element name="tr">
              ...
              <xsl:element name="td">
                <xsl:attribute name="class">detailsHalfTableTd</xsl:attribute>
                <xsl:value-of
                  select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:encounterParticipant[@typeCode='DIS']/cda:assignedEntity/cda:assignedPerson/ext:asEmployment/ext:employerOrganization/cda:name"/>
              </xsl:element>
            </xsl:element>
          </xsl:if>
        </xsl:element> <!-- </tbody> -->
    ...
    <xsl:if test="$participant/cda:associatedEntity/cda:associatedPerson/cda:name and
      ...
      <xsl:element name="table">
        ...
        <xsl:element name="tbody">
          <xsl:call-template name="getContactDetailsRows">
            <xsl:with-param name="personOrOrg"
              select="$participant/cda:associatedEntity"/>
          </xsl:call-template>
            ...
            <xsl:element name="td">
              ...
              <xsl:element name="dl">
                ...
                <xsl:choose>
                  ...
                  <xsl:when
                    test="count($participant/cda:associatedEntity/cda:addr[$showProviderHomeDetails = 'true' or not(contains(@use, 'H'))]) &gt; 0">
                    <xsl:for-each
                      select="$participant/cda:associatedEntity/cda:addr">
          ...
          <xsl:element name="tr">
            ...
            <xsl:element name="td">
              <xsl:attribute name="class">detailsHalfTableTd</xsl:attribute>
              <xsl:for-each
                select="$participant/cda:associatedEntity/cda:associatedPerson/cda:name">
                <xsl:call-template name="show-name">
                  <xsl:with-param name="name" select="."/>
                </xsl:call-template>
                <xsl:if test="position() > 1">
              ...
              <xsl:for-each
                select="$participant/cda:associatedEntity/cda:associatedPerson/ext:asEntityIdentifier">
                <xsl:call-template name="getProviderIndividualIdentifier">
                  <xsl:with-param name="entityIdentifier" select="."/>
                </xsl:call-template>
              </xsl:for-each>
              <xsl:if test="$participant/cda:associatedEntity/cda:associatedPerson/ext:asEmployment[@classCode='EMP']/ext:jobCode/cda:originalText or
                ...
                <xsl:call-template name="getProviderIndividualOccupation">
                  <xsl:with-param name="jobCode"
                    select="$participant/cda:associatedEntity/cda:associatedPerson/ext:asEmployment[@classCode='EMP']/ext:jobCode"/>
                </xsl:call-template>
              </xsl:if>
              <xsl:if test="$participant/cda:associatedEntity/cda:associatedPerson/ext:asQualifications/ext:code/cda:originalText and
                ...
                <xsl:call-template name="getProviderIndividualQualifications">
                  <xsl:with-param name="qualifications"
                    select="$participant/cda:associatedEntity/cda:associatedPerson/ext:asQualifications/ext:code/cda:originalText/text()"/>
                </xsl:call-template>
              </xsl:if>
            </xsl:element>
          </xsl:element>
          ...
          <xsl:if test="($cdaDocumentType=$DH_PATHOLOGY_REPORT_CLINICAL_DOCUMENT_TYPE_NAME or $cdaDocumentType=$DH_DIAGNOSTIC_IMAGING_REPORT_CLINICAL_DOCUMENT_TYPE_NAME) and
                  ...
                  <xsl:otherwise>
                    <xsl:call-template name="show-time">
                      <xsl:with-param name="datetime"
                        select="$participant/cda:time/cda:low"/>
                    </xsl:call-template>
                  </xsl:otherwise>
                  <xsl:when
                    test="($participant/cda:time/@value and string-length($participant/cda:time/@value) &gt; 0)">
                    <xsl:call-template name="show-time">
                      <xsl:with-param name="datetime"
                        select="$participant/cda:time"/>
                    </xsl:call-template>
                  </xsl:when>
                </xsl:choose>
              </xsl:element> <!-- </td> -->
            ...
            <xsl:variable name="orderID">
              <xsl:choose>
                <xsl:otherwise>
                  <xsl:for-each
                    select="/cda:ClinicalDocument/cda:inFulfillmentOf/cda:order/cda:id">
                    <xsl:if test="./@root and
                                            not(starts-with(./@root, '1.2.36.1.2001.1005.53.'))">
                      <xsl:choose>
                        <xsl:otherwise/>
                        <xsl:when
                          test="./@root and string-length(./@root) &gt; 0">
                          <xsl:value-of select="./@root"/>
                        </xsl:when>
                        <xsl:when
                          test="./@extension and string-length(./@extension) &gt; 0">
                          <xsl:value-of select="./@extension"/>
                        </xsl:when>
                      </xsl:choose>
                    </xsl:if>
                ...
                <xsl:when
                  test="$cdaDocumentType=$DH_PATHOLOGY_REPORT_CLINICAL_DOCUMENT_TYPE_NAME">
                  <xsl:choose>
                    <xsl:otherwise/>
                    <xsl:when
                      test="$pathOrderID/@root and string-length($pathOrderID/@root) &gt; 0">
                      <xsl:value-of select="$pathOrderID/@root"/>
                    </xsl:when>
                    <xsl:when
                      test="$pathOrderID/@extension and string-length($pathOrderID/@extension) &gt; 0">
                      <xsl:value-of select="$pathOrderID/@extension"/>
                    </xsl:when>
                  </xsl:choose>
                  <xsl:variable name="pathOrderID"
                    select="/cda:ClinicalDocument/cda:inFulfillmentOf/cda:order/cda:id"/>
                </xsl:when>
              </xsl:choose>
          ...
          <xsl:if test="$participant/cda:associatedEntity/cda:associatedPerson/ext:asEmployment/ext:employerOrganization/cda:asOrganizationPartOf/cda:wholeOrganization/cda:name and
            ...
            <xsl:element name="tr">
              ...
              <xsl:element name="td">
                ...
                <xsl:call-template name="getProviderOrganisationIdentifier">
                  <xsl:with-param name="entityIdentifier"
                    select="$participant/cda:associatedEntity/cda:associatedPerson/ext:asEmployment/ext:employerOrganization/cda:asOrganizationPartOf/cda:wholeOrganization/ext:asEntityIdentifier"/>
                </xsl:call-template>
                <xsl:value-of
                  select="$participant/cda:associatedEntity/cda:associatedPerson/ext:asEmployment/ext:employerOrganization/cda:asOrganizationPartOf/cda:wholeOrganization/cda:name"/>
              </xsl:element>
            </xsl:element>
          </xsl:if>
          ...
          <xsl:if test="$participant/cda:associatedEntity/cda:associatedPerson/ext:asEmployment/ext:employerOrganization/cda:name and
            ...
            <xsl:element name="tr">
              ...
              <xsl:element name="td">
                <xsl:attribute name="class">detailsHalfTableTd</xsl:attribute>
                <xsl:value-of
                  select="$participant/cda:associatedEntity/cda:associatedPerson/ext:asEmployment/ext:employerOrganization/cda:name"/>
              </xsl:element>
            </xsl:element>
          </xsl:if>
        </xsl:element> <!-- </tbody> -->
      ...
      <xsl:element name="table">
        ...
        <xsl:element name="caption">
          ...
          <xsl:choose>
            ...
            <xsl:when
              test="$cdaDocumentType=$DH_MHR_DISPENSE_RECORD_CLINICAL_DOCUMENT_TYPE_NAME">
              Dispensing Organisation
            </xsl:when>
            <xsl:when
              test="$cdaDocumentType=$DH_MHR_PRESCRIPTION_RECORD_CLINICAL_DOCUMENT_TYPE_NAME">
          ...
          <xsl:element name="tr">
            <xsl:element name="th">
              ...
              <xsl:choose>
                ...
                <xsl:when
                  test="$cdaDocumentType=$DH_MHR_DISPENSE_RECORD_CLINICAL_DOCUMENT_TYPE_NAME">
                  Dispensing Organisation Details
                </xsl:when>
                <xsl:when
                  test="$cdaDocumentType=$DH_MHR_PRESCRIPTION_RECORD_CLINICAL_DOCUMENT_TYPE_NAME">
                  Prescriber Organisation Details
                </xsl:when>
              </xsl:choose>
            </xsl:element> <!-- </th> -->
            <xsl:element name="th">
        ...
        <xsl:element name="tbody">
          <xsl:call-template name="getAddressDetailsRows">
            <xsl:with-param name="personOrOrg"
              select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:location/cda:healthCareFacility/cda:serviceProviderOrganization/cda:asOrganizationPartOf/cda:wholeOrganization"/>
          ...
          <xsl:element name="tr">
            ...
            <xsl:element name="td">
              <xsl:attribute name="class">detailsHalfTableTd</xsl:attribute>
              <xsl:for-each
                select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:location/cda:healthCareFacility/cda:serviceProviderOrganization/cda:asOrganizationPartOf/cda:wholeOrganization/ext:asEntityIdentifier">
                <xsl:call-template name="getProviderOrganisationIdentifier">
                  <xsl:with-param name="entityIdentifier" select="."/>
                </xsl:call-template>
              </xsl:for-each>
          ...
          <xsl:for-each
            select="//ext:coverage2[@typeCode='COVBY']/ext:entitlement[@classCode='COV' and @moodCode='EVN' and ext:participant[@typeCode='HLD']/ext:participantRole[@classCode='SDLOC']]">
            <xsl:if
              test="./ext:participant/ext:participantRole/ext:id/@root = /cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:location/cda:healthCareFacility/cda:id/@root">
              ...
              <xsl:element name="tr">
                ...
                <xsl:element name="td">
                  ...
                  <xsl:if test="$entitlementType and
                    ...
                    select="$entitlementType"/>
                  </xsl:if>
                  <xsl:value-of select="$entitlementNumber"/>
                </xsl:element> <!-- </td> -->
              </xsl:element>
              <xsl:variable name="entitlementNumber"
                select="ext:id/@extension"></xsl:variable>

              <xsl:variable name="entitlementType"
                select="ext:code/@displayName"></xsl:variable> <!-- </tr> -->
            </xsl:if>
          </xsl:for-each>
          ...
          <xsl:if test="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:location/cda:healthCareFacility/cda:serviceProviderOrganization/cda:name and
            ...
            <xsl:element name="tr">
              ...
              <xsl:element name="td">
                <xsl:attribute name="class">detailsHalfTableTd</xsl:attribute>
                <xsl:for-each
                  select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:location/cda:healthCareFacility/cda:serviceProviderOrganization/ext:asEntityIdentifier">
                  <xsl:call-template name="getProviderOrganisationIdentifier">
                    <xsl:with-param name="entityIdentifier" select="."/>
                  </xsl:call-template>
                </xsl:for-each>
      ...
      <xsl:element name="table">
              ...
              <xsl:element name="td">
                ...
                <xsl:call-template name="getSignificantParticipant">
                  <xsl:with-param name="participant"
                    select="/cda:ClinicalDocument/cda:participant[@typeCode='REFT']"/>
          ...
          <xsl:when test="$cdaDocumentType=$DH_SPECIALIST_LETTER_CLINICAL_DOCUMENT_TYPE_NAME and
            ...
            <xsl:element name="tr">
              ...
              <xsl:element name="td">
                ...
                <xsl:call-template name="getSignificantParticipant">
                  <xsl:with-param name="participant"
                    select="/cda:ClinicalDocument/cda:participant[@typeCode='REFB']"/>
                  <xsl:with-param name="participantTitle">Referrer
                  </xsl:with-param>
                </xsl:call-template>
              </xsl:element> <!-- </td> -->
              <xsl:element name="td">
          ...
          <xsl:when test="($cdaDocumentType=$DH_PATHOLOGY_REPORT_CLINICAL_DOCUMENT_TYPE_NAME or $cdaDocumentType=$DH_DIAGNOSTIC_IMAGING_REPORT_CLINICAL_DOCUMENT_TYPE_NAME) and
            ...
            <xsl:element name="tr">
              ...
              <xsl:element name="td">
                ...
                <xsl:call-template name="getSignificantParticipant">
                  <xsl:with-param name="participant"
                    select="/cda:ClinicalDocument/cda:participant[@typeCode='REF']"/>
                  <xsl:with-param name="participantTitle">Requester
                  </xsl:with-param>
                </xsl:call-template>
              </xsl:element> <!-- </td> -->
              <xsl:element name="td">
        ...
        <xsl:element name="tr">
          ...
          <xsl:element name="td">
            ...
            <xsl:if
              test="$cdaDocumentType=$DH_MHR_PRESCRIPTION_RECORD_CLINICAL_DOCUMENT_TYPE_NAME or $cdaDocumentType=$DH_MHR_DISPENSE_RECORD_CLINICAL_DOCUMENT_TYPE_NAME">
              <xsl:call-template name="getFacilityDetails"/>
            </xsl:if>
          </xsl:element> <!-- </td> -->
          <xsl:element name="td">
  ...
  <xsl:template name="getPatientDetails">
    <xsl:element name="table">
      ...
      <xsl:element name="tbody">
        ...
        <xsl:for-each
          select="//ext:coverage2[@typeCode='COVBY']/ext:entitlement[@classCode='COV' and @moodCode='EVN' and ext:participant[@typeCode='BEN']/ext:participantRole[@classCode='PAT']]">
          <xsl:if
            test="./ext:participant/ext:participantRole/ext:id/@root = /cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:id/@root">
        ...
        <xsl:for-each
          select="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/ext:asEntityIdentifier[@classCode='IDENT' and ext:id[@assigningAuthorityName!='IHI']]">
              ...
              <xsl:element name="td">
                ...
                <xsl:choose>
                  ...
                  <xsl:when test="$identifierAssigningAuthorityName and
                    ...
                    <xsl:text> (</xsl:text><xsl:text>)</xsl:text><xsl:value-of
                    select="$identifierAssigningAuthorityName"/>
                  </xsl:when>
                </xsl:choose>
        ...
        <xsl:if test="$patientDeceasedStatus='true'">
          <xsl:element name="tr">
            ...
            <xsl:element name="td">
              <xsl:attribute name="class">detailsHalfTableTd</xsl:attribute>
              <xsl:element name="span"><xsl:attribute name="style">font-weight:
                bold;</xsl:attribute>DECEASED
              </xsl:element>
            </xsl:element>
          </xsl:element> <!-- </tr> -->
  ...
  <xsl:template name="getAuthorDetails">
      ...
      <xsl:element name="tbody">
        <xsl:choose>
          <xsl:otherwise>
            <xsl:call-template name="getAddressDetailsRows">
              <xsl:with-param name="personOrOrg"
                select="/cda:ClinicalDocument/cda:author/cda:assignedAuthor"/>
            </xsl:call-template> <!-- </tr> -->
            ...
            <xsl:element name="tr">
              ...
              <xsl:element name="td">
                <xsl:attribute name="class">detailsHalfTableTd</xsl:attribute>
                <xsl:for-each
                  select="/cda:ClinicalDocument/cda:author/cda:assignedAuthor/cda:assignedPerson/cda:name">
                  <xsl:call-template name="show-name">
                    <xsl:with-param name="name" select="."/>
                  </xsl:call-template>
                  <xsl:if test="position() > 1">
                ...
                <xsl:for-each
                  select="/cda:ClinicalDocument/cda:author/cda:assignedAuthor/cda:assignedPerson/ext:asEntityIdentifier">
                  <xsl:call-template name="getProviderIndividualIdentifier">
                    <xsl:with-param name="entityIdentifier" select="."/>
                  </xsl:call-template>
                </xsl:for-each>
                <xsl:if test="/cda:ClinicalDocument/cda:author/cda:assignedAuthor/cda:code/@displayName and
                  ...
                  <xsl:value-of
                    select="/cda:ClinicalDocument/cda:author/cda:assignedAuthor/cda:code/@displayName"/>
                </xsl:if>
              </xsl:element> <!-- </td> -->
            </xsl:element>

            <xsl:for-each
              select="//ext:coverage2[@typeCode='COVBY']/ext:entitlement[@classCode='COV' and @moodCode='EVN' and ext:participant[@typeCode='HLD']/ext:participantRole[@classCode='ASSIGNED']]">
              <xsl:if
                test="./ext:participant/ext:participantRole/ext:id/@root = /cda:ClinicalDocument/cda:author/cda:assignedAuthor/cda:id/@root">

                <xsl:element name="tr">
                  <xsl:element name="th">
                    <xsl:attribute name="class">detailsHalfTableTh
                    </xsl:attribute>
                    <xsl:element name="span">
                      <xsl:attribute name="class">detailsHalfTableThSpan
                      </xsl:attribute>
                  ...
                  <xsl:element name="td">
                    <xsl:attribute name="class">detailsHalfTableTd
                    </xsl:attribute>
                    <xsl:if test="$entitlementType and
                                            string-length($entitlementType) &gt; 0">
                      <xsl:text> (</xsl:text><xsl:text>)</xsl:text><xsl:value-of
            ...
            <xsl:if test="string-length($authorOrg) &gt; 0">
              <xsl:element name="tr">
                ...
                <xsl:element name="td">
                  <xsl:attribute name="class">detailsHalfTableTd</xsl:attribute>
                  <xsl:for-each
                    select="./cda:author/cda:assignedAuthor/cda:assignedPerson/ext:asEmployment/ext:employerOrganization/cda:asOrganizationPartOf/cda:wholeOrganization/ext:asEntityIdentifier">
                    <xsl:call-template name="getProviderOrganisationIdentifier">
                      <xsl:with-param name="entityIdentifier" select="."/>
                    </xsl:call-template>
                  </xsl:for-each>
          ...
          <xsl:when
            ...
            <xsl:element name="tr">
              ...
              <xsl:element name="td">
                <xsl:attribute name="class">detailsHalfTableTd</xsl:attribute>
                <xsl:for-each
                  select="/cda:ClinicalDocument/cda:author/cda:assignedAuthor/cda:assignedAuthoringDevice/ext:asEntityIdentifier">
    ...
    <xsl:variable name="authorDepartment"
      select="/cda:ClinicalDocument/cda:author/cda:assignedAuthor/cda:assignedPerson/ext:asEmployment/ext:employerOrganization/cda:name"/>

    <xsl:variable name="authorOrg"
      select="/cda:ClinicalDocument/cda:author/cda:assignedAuthor/cda:assignedPerson/ext:asEmployment/ext:employerOrganization/cda:asOrganizationPartOf/cda:wholeOrganization/cda:name"/> <!-- </table> -->
  </xsl:template>
  ...
  <xsl:template name="getDocumentDetails">
            ...
            <xsl:element name="td">
              ...
              <xsl:choose>
                ...
                <xsl:when test="$completionCode='Withdrawn'">
                  <xsl:element name="span"><xsl:attribute name="style">
                    font-weight:
                    bold;</xsl:attribute>WITHDRAWN
                  </xsl:element>
                </xsl:when>
              </xsl:choose>
            </xsl:element> <!-- </td> -->
    ...
    <xsl:variable name="documentVersion"
      select="/cda:ClinicalDocument/cda:versionNumber/@value"/>
    ...
    <xsl:variable name="dateTimeAttested">
      <xsl:call-template name="show-time">
        <xsl:with-param name="datetime"
          select="/cda:ClinicalDocument/cda:legalAuthenticator/cda:time"/>
      </xsl:call-template>
    </xsl:variable>

    <xsl:variable name="replacementStatusDocumentId">
      <xsl:call-template name="getDocumentIdentifier">
        <xsl:with-param name="iiVariable"
          select="/cda:ClinicalDocument/cda:relatedDocument[@typeCode='RPLC']/cda:parentDocument/cda:id"/>
      </xsl:call-template>
    </xsl:variable>
    ...
    <xsl:variable name="transformStatusDocumentId">
      <xsl:call-template name="getDocumentIdentifier">
        <xsl:with-param name="iiVariable"
          select="/cda:ClinicalDocument/cda:relatedDocument[@typeCode='XFRM']/cda:parentDocument/cda:id"/>
      </xsl:call-template>
    </xsl:variable>
  ...
  <xsl:template name="getParticipantsTable">
      ...
      <xsl:element name="colgroup">
        <xsl:element name="col">
          <xsl:attribute name="style">width: <xsl:value-of
            select="$FIFTH_CONTENT_WIDTH_PCT"/>%;
          </xsl:attribute>
        </xsl:element> <!-- </col> -->
        <xsl:element name="col">
          <xsl:attribute name="style">width: <xsl:value-of
        ...
        <xsl:for-each select="$participants">
          <xsl:choose>
            ...
            <xsl:when test="./@typeCode='IRCP'">
              <xsl:call-template name="getParticipantsTableBodyRow">
                <xsl:with-param name="participantType">Patient's Nominated Contact
                </xsl:with-param>
              </xsl:call-template>
            </xsl:when>
            <xsl:when test="./@typeCode='PART' and cda:functionCode/@code='PCP'">
              <xsl:choose>
                <xsl:otherwise>
                  <xsl:call-template name="getParticipantsTableBodyRow">
                    <xsl:with-param name="participantType">Usual GP
                    </xsl:with-param>
                  </xsl:call-template>
                </xsl:otherwise>
                <xsl:when
                  test="$cdaDocumentType=$DH_DISCHARGE_SUMMARY_CLINICAL_DOCUMENT_TYPE_NAME">
                  <xsl:call-template name="getParticipantsTableBodyRow">
                    <xsl:with-param name="participantType">Nominated Primary
                      Healthcare Provider
                      Person
                    </xsl:with-param>
                  </xsl:call-template>
    ...
    <xsl:param name="title"></xsl:param> <!-- </table> -->
  </xsl:template>
  <xsl:template name="getRecipientsTable">
    ...
    <xsl:element name="table">
      ...
      <xsl:element name="colgroup">
        <xsl:element name="col">
          <xsl:attribute name="style">width: <xsl:value-of
            select="$QUARTER_CONTENT_WIDTH_PCT"/>%;
          </xsl:attribute>
        </xsl:element> <!-- </col> -->
        <xsl:element name="col">
          <xsl:attribute name="style">width: <xsl:value-of
  ...
  <xsl:template name="getParticipantsTableBodyRow">
    <xsl:element name="tr">
      <xsl:element name="td">
        ...
        <xsl:for-each
          select="./cda:associatedEntity/cda:associatedPerson/ext:asEntityIdentifier">
          <xsl:call-template name="getProviderIndividualIdentifier">
            <xsl:with-param name="entityIdentifier" select="."/>
          </xsl:call-template>
        </xsl:for-each>
        <xsl:if test="./cda:associatedEntity/cda:associatedPerson/ext:asQualifications/ext:code/cda:originalText and
          ...
          <xsl:call-template name="getProviderIndividualQualifications">
            <xsl:with-param name="qualifications"
              select="./cda:associatedEntity/cda:associatedPerson/ext:asQualifications/ext:code/cda:originalText/text()"/>
          </xsl:call-template>
        </xsl:if>
        <xsl:if test="./cda:associatedEntity/cda:associatedPerson/ext:asEmployment[@classCode='EMP']/ext:jobCode/cda:originalText or
          ...
          <xsl:call-template name="getProviderIndividualOccupation">
            <xsl:with-param name="jobCode"
              select="./cda:associatedEntity/cda:associatedPerson/ext:asEmployment[@classCode='EMP']/ext:jobCode"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:element> <!-- </td> -->
      ...
      <xsl:element name="td">
        ...
        <xsl:choose>
          ...
          <xsl:when
            test="count(./cda:associatedEntity/cda:addr[$showProviderHomeDetails = 'true' or not(contains(@use, 'H'))]) &gt; 0">
            <xsl:element name="dl">
              <xsl:attribute name="class">addressdl</xsl:attribute>
      ...
      <xsl:element name="td">
        <xsl:attribute name="class">PARTICIPANT_BODY_ROW_TD</xsl:attribute>
        <xsl:for-each
          select="./cda:associatedEntity/cda:associatedPerson/ext:asEmployment/ext:employerOrganization/cda:asOrganizationPartOf/cda:wholeOrganization">
          <xsl:element name="br"/>
      ...
      <xsl:element name="td">
        <xsl:attribute name="class">PARTICIPANT_BODY_ROW_TD</xsl:attribute>
        <xsl:for-each
          select="./cda:associatedEntity/cda:associatedPerson/ext:asEmployment/ext:employerOrganization">
          <xsl:element name="br"/>
          <xsl:for-each select="./ext:asEntityIdentifier">
            <xsl:call-template name="getProviderOrganisationIdentifier">
              <xsl:with-param name="entityIdentifier" select="."/>
    ...
    <xsl:param name="participantType"/> <!-- </tr> -->
  </xsl:template>
    ...
    <xsl:element name="tr">
      <xsl:element name="td">
        ...
        <xsl:if test="./cda:informationRecipient/ext:asQualifications/ext:code/cda:originalText and
          ...
          <xsl:call-template name="getProviderIndividualQualifications">
            <xsl:with-param name="qualifications"
              select="./cda:informationRecipient/ext:asQualifications/ext:code/cda:originalText/text()"/>
          </xsl:call-template>
        </xsl:if>
        <xsl:if test="./cda:informationRecipient/ext:asEmployment[@classCode='EMP']/ext:jobCode/cda:originalText or
          ...
          <xsl:call-template name="getProviderIndividualOccupation">
            <xsl:with-param name="jobCode"
              select="./cda:informationRecipient/ext:asEmployment[@classCode='EMP']/ext:jobCode"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:element> <!-- </td> -->
      ...
      <xsl:element name="td">
        ...
        <xsl:choose>
          ...
          <xsl:when
            test="count(./cda:addr[$showProviderHomeDetails = 'true' or not(contains(@use, 'H'))]) &gt; 0">
            <xsl:element name="dl">
              <xsl:attribute name="class">addressdl</xsl:attribute>
      ...
      <xsl:element name="td">
        ...
        <xsl:for-each select="./cda:receivedOrganization">
          ...
          <xsl:for-each
            select="./cda:asOrganizationPartOf/cda:wholeOrganization/ext:asEntityIdentifier">
            <xsl:call-template name="getProviderOrganisationIdentifier">
              <xsl:with-param name="entityIdentifier" select="."/>
            </xsl:call-template>
          </xsl:for-each>
    ...
    <xsl:choose>
      <xsl:otherwise>
        <xsl:for-each select="$structuredBody/cda:component/cda:section">
          <xsl:if
            test="not((cda:code/@code='102.16080') and (cda:code/@codeSystem='1.2.36.1.2001.1001.101'))">
    ...
    <xsl:choose>
      <xsl:otherwise>
        <xsl:choose>
          <xsl:otherwise>
            <xsl:value-of
              select="concat('Unlabeled Section', $SECTION_TITLE_NAVIGATION_SEPARATOR, $previousSectionTitle)"/>
          </xsl:otherwise>
          <xsl:when test="$section/cda:title and
                        string-length($section/cda:title) &gt; 0">
            <xsl:value-of
              select="concat($section/cda:title, $SECTION_TITLE_NAVIGATION_SEPARATOR, $previousSectionTitle)"/>
          </xsl:when>
        </xsl:choose>
      </xsl:otherwise>
          ...
          <xsl:if test="$sectionId and
            ...
            <xsl:element name="div">
              ...
              <xsl:attribute name="style">
                <xsl:value-of
                  select="concat('position:relative; top:-', $BANNER_HEIGHT_PX, 'px;')"/>
              </xsl:attribute>
            </xsl:element>
          </xsl:if>
          <xsl:value-of select="$title"/>
      ...
      <xsl:when test="contains($title, $SECTION_TITLE_NAVIGATION_SEPARATOR)">
        ...
        <xsl:element name="h1">
          ...
          <xsl:choose>
            ...
            <xsl:when
              test="$actualSectionTitle = 'Clinical Interventions Performed This Visit'">
              Procedures
            </xsl:when>
        ...
        <xsl:variable name="actualSectionTitle">
          <xsl:call-template name="substring-after-last-occurance">
            ...
            <xsl:with-param name="argument"
              select="$SECTION_TITLE_NAVIGATION_SEPARATOR"/>
          </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="leadIn">
          <xsl:value-of
            select="concat(substring-before($title, concat($SECTION_TITLE_NAVIGATION_SEPARATOR, $actualSectionTitle)), $SECTION_TITLE_NAVIGATION_SEPARATOR)"/>
        </xsl:variable>
      </xsl:when>
    </xsl:choose>
  ...
  <xsl:template match="cda:footnote">
    <xsl:element name="sup">
      <xsl:element name="a">
        <xsl:attribute name="href">#F_footnote-<xsl:value-of select="$footnote-num"/>
        </xsl:attribute>
    ...
    <xsl:variable name="footnote-num">
      <xsl:number count="cda:footnote" level="any"/>
    </xsl:variable> <!-- </sup> -->
  </xsl:template>
  <xsl:template match="cda:footnote" mode="footnote">
    <xsl:element name="p">
      ...
      <xsl:element name="span">
        ...
        <xsl:text>[</xsl:text><xsl:text>] </xsl:text><xsl:value-of select="$footnote-num"/>
      </xsl:element>
      <xsl:element name="div">
        <xsl:attribute name="id">F_footnote-<xsl:value-of select="$footnote-num"/>
        </xsl:attribute>
        <xsl:attribute name="name">F_footnote-<xsl:value-of select="$footnote-num"/>
        </xsl:attribute>
        <xsl:attribute name="style">
          <xsl:value-of
            select="concat('position:relative; top:-', $BANNER_HEIGHT_PX, 'px;')"/>
  ...
  <xsl:template match="cda:footnoteRef">
    <xsl:element name="sup">
      <xsl:element name="a">
        <xsl:attribute name="href">#<xsl:value-of
          select="concat('F_footnote-',$footnote-num)"/>
        </xsl:attribute>
        <xsl:attribute name="title">ref#<xsl:value-of select="$footnote-num"/>
        </xsl:attribute>
        <xsl:value-of select="$footnote-num"/>
      </xsl:element> <!-- </a> -->
    </xsl:element>
    ...
    <xsl:variable name="theFootnote"
      select="//cda:footnote[@ID=current()/@IDREF]"/> <!-- </sup> -->
  </xsl:template>
            ...
            <xsl:when test="string-length(./@href) &gt; 1 and
              ...
              <xsl:choose>
                ...
                <xsl:when test="$attachmentReference='true'">
                  <xsl:if test="//cda:observationMedia/@ID=$referenceId">
                    <xsl:value-of
                      select="//cda:observationMedia[@ID=$referenceId]/cda:value/cda:reference/@value"/>
                  </xsl:if>
                </xsl:when>
              </xsl:choose>
              <xsl:variable name="attachmentReference">
                <xsl:choose>
                  ...
                  <xsl:when test="//cda:observationMedia/@ID=$referenceId">true
                  </xsl:when>
                </xsl:choose>
              </xsl:variable>
                ...
                <xsl:element name="li">
                  <xsl:choose>
                    ...
                    <xsl:when test="@styleCode">
                      <xsl:call-template name="processStyleCode">
                        <xsl:with-param
                          name="initialCssClasses"></xsl:with-param>
                        <xsl:with-param name="code" select="@styleCode"/>
                      </xsl:call-template>
                    </xsl:when>
                  </xsl:choose>
      ...
      <xsl:when test="@styleCode">
        <xsl:choose>
          ...
          <xsl:when test="@styleCode='BigRoman'">
            <xsl:element name="ol">
              <xsl:attribute name="style">list-style-type: upper-roman;
              </xsl:attribute>
              <xsl:attribute name="class">genericol</xsl:attribute>
              <xsl:call-template name="addID">
                <xsl:with-param name="element" select="."/>
          ...
          <xsl:when test="@styleCode='LittleAlpha'">
            <xsl:element name="ol">
              <xsl:attribute name="class">genericol</xsl:attribute>
              <xsl:attribute name="style">list-style-type: lower-alpha;
              </xsl:attribute>
              <xsl:call-template name="addID">
                <xsl:with-param name="element" select="."/>
              </xsl:call-template>
          ...
          <xsl:when test="@styleCode='BigAlpha'">
            <xsl:element name="ol">
              <xsl:attribute name="style">list-style-type: upper-alpha;
              </xsl:attribute>
              <xsl:attribute name="class">genericol</xsl:attribute>
              <xsl:call-template name="addID">
                <xsl:with-param name="element" select="."/>
          ...
          <xsl:when test="@styleCode='LittleRoman'">
            <xsl:element name="ol">
              <xsl:attribute name="class">genericol</xsl:attribute>
              <xsl:attribute name="style">list-style-type: lower-roman;
              </xsl:attribute>
              <xsl:call-template name="addID">
                <xsl:with-param name="element" select="."/>
              </xsl:call-template>
      ...
      <xsl:choose>
        ...
        <xsl:when test="./@styleCode">
          <xsl:choose>
            ...
            <xsl:when
              test="contains(./@styleCode, 'rule') or contains(./@styleCode, 'Rule')">
              <xsl:call-template name="processStyleCode">
    ...
    <xsl:attribute name="class">
      <xsl:call-template name="trim">
        <xsl:with-param name="string">
          <xsl:value-of
            select="concat($initialCssClasses, ' ', $boldClass, ' ', $italicClass, ' ', $underlineClass, ' ', $emphasisClass, ' ', $xFixedClass)"/>
        </xsl:with-param>
      </xsl:call-template>
    </xsl:attribute>
    <xsl:choose>
    ...
    <xsl:if test="(string-length($initialStyles) &gt; 0) or
      ...
      <xsl:attribute name="style">
        <xsl:call-template name="trim">
          <xsl:with-param name="string">
            <xsl:value-of
              select="concat($initialStyles, ' ', $colourStyles, ' ', $fontSizePxStyle, ' ', $fontSizeEmStyle)"/>
          </xsl:with-param>
        </xsl:call-template>
      </xsl:attribute>
    </xsl:if>
    ...
    <xsl:variable name="tableCellClasses">
      <xsl:choose>
        ...
        <xsl:when test="string-length($lRuleClass) &gt; 0">
          <xsl:text> </xsl:text><xsl:value-of select="$lRuleClass"/>
        </xsl:when>
      </xsl:choose>
      <xsl:choose>
        <xsl:otherwise>noRrule</xsl:otherwise>
        <xsl:when test="string-length($rRuleClass) &gt; 0">
          <xsl:text> </xsl:text><xsl:value-of select="$rRuleClass"/>
        </xsl:when>
      </xsl:choose>
      ...
      <xsl:choose>
        ...
        <xsl:when test="string-length($topruleClass) &gt; 0">
          <xsl:text> </xsl:text><xsl:value-of select="$topruleClass"/>
        </xsl:when>
      </xsl:choose>
      ...
      <xsl:choose>
        ...
        <xsl:when test="string-length($botruleClass) &gt; 0">
          <xsl:text> </xsl:text><xsl:value-of select="$botruleClass"/>
        </xsl:when>
      </xsl:choose>

      <xsl:if test="$initialCssClasses and string-length($initialCssClasses) &gt; 0">
        <xsl:text> </xsl:text><xsl:value-of select="$initialCssClasses"/>
      </xsl:if>
    </xsl:variable>
  ...
  <xsl:template name="processColourStyleCode">
    ...
    <xsl:variable name="validFgColourHexCode">
      <xsl:call-template name="validatateColourStyleCodeValue">
        <xsl:with-param name="styleCodeValue" select="$fgColourHexCode"/>
      </xsl:call-template>
    </xsl:variable>
    ...
    <xsl:variable name="validBgColourHexCode">
      <xsl:call-template name="validatateColourStyleCodeValue">
        <xsl:with-param name="styleCodeValue" select="$bgColourHexCode"/>
      </xsl:call-template>
    </xsl:variable>
  ...
  <xsl:template name="processFontSizePx">
    ...
    <xsl:variable name="validPxFontSize">
      <xsl:call-template name="validatateNumericValue">
        <xsl:with-param name="value" select="$fontSize"/>
      </xsl:call-template>
    </xsl:variable>
  ...
  <xsl:template name="processColWidthPx">
    ...
    <xsl:variable name="validColWidth">
      <xsl:call-template name="validatateNumericValue">
  ...
  <xsl:template name="processFontSizeEm">
    ...
    <xsl:variable name="validEmFontSize">
      <xsl:call-template name="validatateNumericValue">
        <xsl:with-param name="value" select="$fontSize"/>
      </xsl:call-template>
    </xsl:variable>
    ...
    <xsl:choose>
      ...
      <xsl:when test="string-length($styleCodeValue) = 6">
        <xsl:choose>
          ...
          <xsl:when
            test="string-length(translate($styleCodeValue, $validHexColourCharacters, '')) = 0">
  ...
  <xsl:template name="validatateNumericValue">
    <xsl:choose>
      ...
      <xsl:when test="string-length(translate($value, $validNumericCharacters, '')) = 0">
        <xsl:value-of select="'true'"/>
      </xsl:when>
    </xsl:choose>
  ...
  <xsl:template name="processAttachment">
    <xsl:choose>
      <xsl:otherwise>
        <xsl:if
          test="$mediaType='image/gif' or $mediaType='image/jpeg' or $mediaType='image/png' or $mediaType='image/bmp'">
                ...
                <xsl:element name="tr">
                  <xsl:element name="td">
                    ...
                    <xsl:element name="img">
                      <xsl:attribute name="src">
                        <xsl:value-of
                          select="//cda:observationMedia[@ID=$referenceObjectId]/cda:value/cda:reference/@value"/>
                      </xsl:attribute>
                      <xsl:attribute name="style">padding-bottom:0.5em;
                      </xsl:attribute>
            ...
            <xsl:choose>
              <xsl:otherwise>
                <xsl:choose>
                  ...
                  <xsl:when
                    test="$mediaType='text/rtf' or $mediaType='application/rtf'">
                    RTF
                    document
                  </xsl:when>
                  <xsl:when test="$mediaType='text/plain'">Text document
                  </xsl:when>
                  <xsl:when test="$mediaType='text/html'">HTML document</xsl:when>
                  <xsl:when test="$mediaType='application/xhtml+xml'">XHTML
                    document
                  </xsl:when>
                  <xsl:when test="$mediaType='application/xml'">XML document
                  </xsl:when>
                  <xsl:when test="$mediaType='image/tiff'">TIFF document
                  </xsl:when>
                  <xsl:when test="$mediaType='application/pdf'">PDF document
                  </xsl:when>
                </xsl:choose>
              </xsl:otherwise>
              <xsl:when test="cda:caption and
    ...
    <xsl:variable name="mediaType">
      <xsl:choose>
        ...
        <xsl:when
          test="//cda:observationMedia[@ID=$referenceObjectId]/cda:value/@mediaType">
          <xsl:value-of
            select="//cda:observationMedia[@ID=$referenceObjectId]/cda:value/@mediaType"/>
        </xsl:when>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="linkTargetContainsJavaScript">
  ...
  <xsl:template name="renderLogo">
    <xsl:if
      test="//cda:observationMedia[@ID='LOGO']/cda:value[@mediaType='image/png' or @mediaType='image/gif' or @mediaType='image/jpeg']/cda:reference/@value">
      ...
      <xsl:if test="$logoTargetContainsJavaScript = 'false'">
        <xsl:element name="img">
          ...
          <xsl:attribute name="src">
            <xsl:choose>
              <xsl:when
                test="//cda:observationMedia[@ID='LOGO']/cda:value/@mediaType='image/png'">
                <xsl:value-of
                  select="//cda:observationMedia[@ID='LOGO']/cda:value[@mediaType='image/png']/cda:reference/@value"/>
              </xsl:when>
              <xsl:when
                test="//cda:observationMedia[@ID='LOGO']/cda:value/@mediaType='image/gif'">
                <xsl:value-of
                  select="//cda:observationMedia[@ID='LOGO']/cda:value[@mediaType='image/gif']/cda:reference/@value"/>
              </xsl:when>
              <xsl:when
                test="//cda:observationMedia[@ID='LOGO']/cda:value/@mediaType='image/jpeg'">
                <xsl:value-of
                  select="//cda:observationMedia[@ID='LOGO']/cda:value[@mediaType='image/jpeg']/cda:reference/@value"/>
              </xsl:when>
            </xsl:choose>
          </xsl:attribute>
        </xsl:element> <!-- </img> -->
      ...
      <xsl:variable name="logoTargetContainsJavaScript">
        <xsl:call-template name="containsJavaScript">
          <xsl:with-param name="reference"
            select="//cda:observationMedia[@ID='LOGO']/cda:value[@mediaType='image/png' or @mediaType='image/gif' or @mediaType='image/jpeg']/cda:reference/@value"/>
        </xsl:call-template>
      </xsl:variable>
    </xsl:if>
  </xsl:template>
  <xsl:template name="addCSS">
    <xsl:element name="style">
      ...
      * html .clearfix {height: 1%;}
      ...
      border-bottom: 1px SOLID #000000;
      padding: 0px;
      min-width: <xsl:value-of
      select="$MINIMUM_CONTENT_WIDTH_PX - ($LEFT_MARGIN_WIDTH_PX + $RIGHT_MARGIN_WIDTH_PX)"/>px;
      max-width: <xsl:value-of
      select="$FULL_CONTENT_WIDTH_PX - ($LEFT_MARGIN_WIDTH_PX + $RIGHT_MARGIN_WIDTH_PX)"/>px;
    ...
    <xsl:element name="style">
      ...
      min-width: <xsl:value-of
      select="$MINIMUM_CONTENT_WIDTH_PX - ($RIGHT_MARGIN_WIDTH_PX + $LEFT_MARGIN_WIDTH_PX)"/>px;
      max-width: <xsl:value-of
      select="$FULL_CONTENT_WIDTH_PX - ($RIGHT_MARGIN_WIDTH_PX + $LEFT_MARGIN_WIDTH_PX)"/>px;
      width: 100%;
      }
  ...
  <xsl:template name="getFootnotesTable">
    <xsl:if test="count(//cda:footnote) &gt; 0">
      ...
      <xsl:element name="table">
        <xsl:attribute name="class">participantsTableClass</xsl:attribute>
        <xsl:attribute name="style">table-layout: fixed; word-wrap: break-word;
        </xsl:attribute>
        <xsl:element name="caption">
          <xsl:attribute name="class">sr-only-pdf</xsl:attribute>
          Foot
  ...
  <xsl:template name="getSpaceToScrollToFootnotesDiv">
    <xsl:if test="count(//cda:footnote) &gt; 0 or
      ...
      <xsl:element name="div">
        ...
        <xsl:attribute name="style">height:350px; background:#FFFFFF; color:#FFFFFF;
        </xsl:attribute>
  ...
  <xsl:template name="getRenderingSpecificationIncompatibilityWarning">
    <xsl:call-template name="getDocumentIndicator">
      <xsl:with-param name="indicatorText">
  ...
  <xsl:template name="getDocumentIndicator">
    <xsl:if test="$indicatorText and
      ...
      <xsl:element name="div">
        <xsl:attribute name="style">border: 0px SOLID #000000;
          <xsl:choose>
            ...
            <xsl:when test="$marginTop and string-length($marginTop) &gt; 0">margin-top:
              <xsl:value-of select="$marginTop"/>px;
            </xsl:when>
          </xsl:choose>
          <xsl:choose>
            ...
            <xsl:when test="$marginBottom and string-length($marginBottom) &gt; 0">
              margin-bottom:
              <xsl:value-of select="$marginBottom"/>px;
            </xsl:when>
          </xsl:choose>
          <xsl:choose>
            ...
            <xsl:when test="$paddingBottom and string-length($paddingBottom) &gt; 0">
              padding-bottom:
              <xsl:value-of select="$paddingBottom"/>px;
            </xsl:when>
          </xsl:choose>
          <xsl:choose>
            ...
            <xsl:when test="$paddingTop and string-length($paddingTop) &gt; 0">
              padding-top:
              <xsl:value-of select="$paddingTop"/>px;
            </xsl:when>
          </xsl:choose>
          <xsl:if test="string-length($backgroundColour) &gt; 0">background:
            #<xsl:value-of
              select="$backgroundColour"/>;
          </xsl:if>
        ...
        <xsl:choose>
          <xsl:otherwise>
            <xsl:attribute name="class">documentindicator documentindicatorcolours
            </xsl:attribute>
          </xsl:otherwise>
          <xsl:when
          ...
          <xsl:when test="string-length($borderColour) &gt; 0">
            <xsl:element name="div">
              <xsl:attribute name="style">padding: 0px; margin: 0px; border: 2px SOLID
                #<xsl:value-of select="$borderColour"/>;
              </xsl:attribute>
              <xsl:value-of select="$indicatorText"/>
            </xsl:element> <!-- </div> -->
          </xsl:when>
  ...
  <xsl:template name="getAddressDetailsRows">
    ...
    <xsl:variable name="useOrgAddress"
      select="count($personOrOrgOrgAddress) &gt; 0 and count($personOrOrgAddress) = 0"/>
  ...
  <xsl:template name="getContactDetailsRows">
            ...
            <xsl:element name="th">
              ...
              <xsl:element name="span">
                ...
                <xsl:choose>
                  ...
                  <xsl:when test="starts-with(./@value, 'mailto:') ">Email
                  </xsl:when>
    ...
    <xsl:variable name="useOrgContacts"
      select="count($personOrOrgOrgContacts) &gt; 0 and count($personOrOrgContacts) = 0"/>
  ...
  <xsl:template name="getContactDetails">
    ...
    <xsl:param name="personOrOrg"/>
    <xsl:variable name="faxRows"
      select="$personOrOrg/cda:telecom[starts-with(@value, 'fax:') and ($showProviderHomeDetails = 'true' or not(contains(@use, 'H')))]"/>
    <xsl:variable name="emailRows"
      select="$personOrOrg/cda:telecom[starts-with(@value, 'mailto:') and ($showProviderHomeDetails = 'true' or not(contains(@use, 'H')))]"/>

    <xsl:variable name="telRows"
      select="$personOrOrg/cda:telecom[starts-with(@value, 'tel:') and ($showProviderHomeDetails = 'true' or not(contains(@use, 'H')))]"/>
  ...
  <xsl:template name="getAddress">
    <xsl:if test="not(contains($address/@use, 'H')) or $showProviderHomeDetails = 'true'">
      <xsl:choose>
        <xsl:otherwise>
          <xsl:element name="dt">
            <xsl:attribute name="class">addressdt</xsl:attribute>
            <xsl:call-template name="getAddressUseDescriptor">
          ...
          <xsl:element name="dd">
            <xsl:attribute name="class">addressdd</xsl:attribute>
        ...
        <xsl:when test="$address/@nullFlavor='NA'">
          <xsl:element name="dt"><xsl:attribute name="class">addressdt</xsl:attribute>No
            Fixed
            Address
          </xsl:element>
        </xsl:when>
    ...
    <xsl:if test="$address">
      <xsl:choose>
        <xsl:otherwise>
          ...
          <xsl:if
            test="$address/cda:deliveryAddressLine and string-length($address/cda:deliveryAddressLine)>0">
              ...
              <xsl:if test="string-length(./text())>0">
                <xsl:choose>
                  <xsl:otherwise>
                    <xsl:element name="br"/>
                    <xsl:value-of
                      select="concat('Postal Delivery Number: ', ./text())"/>
                  </xsl:otherwise>
          ...
          <xsl:if test="$address/cda:additionalLocator">
            <xsl:for-each
              select="$address/cda:additionalLocator[string-length(text()) &gt; 0]">
              <xsl:if test="position() &gt; 1">
                <xsl:text>, </xsl:text>
              </xsl:if>
              <xsl:value-of select="."/>
            ...
            <xsl:if
              test="count($address/cda:streetAddressLine[string-length(text()) &gt; 0]) &gt; 0 or
          ...
          <xsl:if test="$address/cda:streetAddressLine">
            <xsl:for-each
              select="$address/cda:streetAddressLine[string-length(text()) &gt; 0]">
              <xsl:if test="position() &gt; 1">
                <xsl:text>, </xsl:text>
              </xsl:if>
              <xsl:value-of select="."/>
          ...
          <xsl:if
            test="$address/cda:houseNumber and string-length($address/cda:houseNumber) &gt; 0">
            <xsl:choose>
              <xsl:otherwise>
              ...
              <xsl:when
                test="$address/cda:streetName and string-length($address/cda:streetName) &gt; 0">
                <xsl:text> </xsl:text>
              </xsl:when>
            </xsl:choose>
          ...
          <xsl:if
            ...
            <xsl:choose>
              ...
              <xsl:when
                test="($address/cda:streetNameType and string-length($address/cda:streetNameType) &gt; 0)
            ...
            <xsl:if
              ...
              <xsl:choose>
                ...
                <xsl:when
                  test="$address/cda:direction and string-length($address/cda:direction) &gt; 0">
                  <xsl:text> </xsl:text>
                </xsl:when>
              </xsl:choose>
        ...
        <xsl:when test="string-length($address/@nullFlavor) &gt; 0">
          <xsl:call-template name="show-null-flavor-value">
            <xsl:with-param name="nullFlavorValue"
              select="$address/@nullFlavor"></xsl:with-param>
          </xsl:call-template>
        </xsl:when>
      </xsl:choose>
    </xsl:if>
  ...
  <xsl:template name="show-telecom">
    <xsl:choose>
      ...
      <xsl:when test="string-length($telecom/@nullFlavor) &gt; 0">
        <xsl:call-template name="show-null-flavor-value">
          <xsl:with-param name="nullFlavorValue"
            select="$telecom/@nullFlavor"></xsl:with-param>
        </xsl:call-template>
      </xsl:when>
    </xsl:choose>
  ...
  <xsl:template name="formatDateTime">
    <xsl:choose>
      ...
      <xsl:when test="string-length(substring ($date, 7, 1)) &gt; 0">
        <xsl:text> </xsl:text><xsl:value-of select="substring ($date, 7, 2)"/>
      </xsl:when>
      <xsl:when test="substring ($date, 7, 1)='0'">
        <xsl:text> </xsl:text><xsl:value-of select="substring ($date, 8, 1)"/>
      </xsl:when>
    </xsl:choose>
      ...
      <xsl:choose>
        ...
        <xsl:when test="starts-with($entityIdentifier/ext:id/@root, '1.2.36.1.2001.1003.0.') and
          ...
          <xsl:element name="div">
            ...
            <xsl:text>[HPI-I: </xsl:text><xsl:text>]</xsl:text>
          </xsl:element>
          <xsl:variable name="identifier">
            <xsl:value-of
              select="substring-after($entityIdentifier/ext:id/@root, '1.2.36.1.2001.1003.0.')"/>
          </xsl:variable>
        </xsl:when>
      </xsl:choose>
    </xsl:if>
  ...
  <xsl:template name="getProviderIndividualOccupation">
    <xsl:choose>
      <xsl:otherwise>
        <xsl:if
          test="$jobCode/@displayName and string-length($jobCode/@displayName) &gt; 0">
          <xsl:element name="div">
            <xsl:attribute name="class">
              <xsl:value-of select="$PROVIDER_INDIVIDUAL_OCCUPATION_CSS_CLASS"/>
            </xsl:attribute>
            ...
            <xsl:value-of select="$jobCode/@displayName"/>
          </xsl:element>
        </xsl:if>
      </xsl:otherwise>
      <xsl:when
        test="$jobCode/cda:originalText and string-length($jobCode/cda:originalText) &gt; 0">
        <xsl:element name="div">
          <xsl:attribute name="class">
            <xsl:value-of select="$PROVIDER_INDIVIDUAL_OCCUPATION_CSS_CLASS"/>
          </xsl:attribute>
          ...
          <xsl:value-of select="$jobCode/cda:originalText/text()"/>
        </xsl:element>
      </xsl:when>
    </xsl:choose>
    ...
    <xsl:if test="$providerDeviceIdentifierDisplay='true' and
      ...
      <xsl:if test="starts-with($entityIdentifier/ext:id/@root, '1.2.36.1.2001.1007.20.') and
        ...
        <xsl:element name="div">
          ...
          <xsl:text>[PAI-D: </xsl:text><xsl:text>]</xsl:text>
        </xsl:element>
        <xsl:variable name="identifier">
          <xsl:value-of
            select="substring-after($entityIdentifier/ext:id/@root, '1.2.36.1.2001.1007.20.')"/>
        </xsl:variable>
      </xsl:if>
    </xsl:if>
    <xsl:param name="entityIdentifier"/>
      ...
      <xsl:choose>
        ...
        <xsl:when test="starts-with($entityIdentifier/ext:id/@root, '1.2.36.1.2001.1007.1.') and
          ...
          <xsl:element name="div">
            ...
            <xsl:text>[PAI-O: </xsl:text><xsl:text>]</xsl:text>
          </xsl:element>
          <xsl:variable name="identifier">
            <xsl:value-of
              select="substring-after($entityIdentifier/ext:id/@root, '1.2.36.1.2001.1007.1.')"/>
          </xsl:variable>
        </xsl:when>
        ...
        <xsl:when test="starts-with($entityIdentifier/ext:id/@root, '1.2.36.1.2001.1003.0.') and
          ...
          <xsl:element name="div">
            ...
            <xsl:text>[HPI-O: </xsl:text><xsl:text>]</xsl:text>
          </xsl:element>
          <xsl:variable name="identifier">
            <xsl:value-of
              select="substring-after($entityIdentifier/ext:id/@root, '1.2.36.1.2001.1003.0.')"/>
  ...
  <xsl:template name="getDocumentIdentifier">
    <xsl:choose>
      <xsl:when
        test="string-length($iiVariable/@root) &gt; 0 and string-length($iiVariable/@extension) &gt; 0 and string-length($iiVariable/@assigningAuthorityName) &gt; 0">
        <xsl:text> (</xsl:text><xsl:text>)</xsl:text><xsl:value-of
        select="$iiVariable/@assigningAuthorityName"/><xsl:value-of
        select="$iiVariable/@extension"/>
      </xsl:when>
      <xsl:when
        test="string-length($iiVariable/@root) &gt; 0 and string-length($iiVariable/@extension) &gt; 0">
        <xsl:text> (</xsl:text><xsl:text>)</xsl:text><xsl:value-of
        select="$iiVariable/@root"/><xsl:value-of
        select="$iiVariable/@extension"/>
      </xsl:when>
      <xsl:when
        test="string-length($iiVariable/@root) &gt; 0 and string-length($iiVariable/@assigningAuthorityName) &gt; 0">
        <xsl:text> (</xsl:text><xsl:text>)</xsl:text><xsl:value-of
        select="$iiVariable/@assigningAuthorityName"/><xsl:value-of
        select="$iiVariable/@root"/>
      </xsl:when>
  ...
  <xsl:template name="containsJavaScript">
    <xsl:choose>
      <xsl:otherwise>false</xsl:otherwise>
      <xsl:when
        test="contains($upperCaseReference, 'JAVASCRIPT') or contains($upperCaseReference, 'JSCRIPT')">
        true
      </xsl:when>
    </xsl:choose>
  ...
  <xsl:variable name="DH_CDA_RENDERING_SPECIFICATION_OID">1.2.36.1.2001.1001.100.149
  </xsl:variable>

  <xsl:variable name="DH_GENERIC_CDA_STYLESHEET_VERSION">1.6.0 - alpha2</xsl:variable>

  <xsl:variable name="VERSION_NOTE">HTML Generated by the Australian Digital Health Agency Generic
    CDA Stylesheet Version:
    <xsl:value-of select="$DH_GENERIC_CDA_STYLESHEET_VERSION"/>
  </xsl:variable>

  <xsl:variable name="DO_NOT_EDIT_NOTE">Do NOT edit this HTML directly: it was generated via an
    XSLT
    transformation from a source CDA Release 2 XML document.
  </xsl:variable>

  <xsl:variable name="CDA_RENDERING_SPECIFICATION_INCOMPATIBILITY_WARNING">The document you are
    about to view is newer than this software was designed to display. The document may not be
    fully
    displayed as the original author intended.
  </xsl:variable>

  <!-- Discharge Details -->
  <xsl:variable name="PROVIDER_INDIVIDUAL_IDENTIFIER_CSS_CLASS">PROVIDER_INDIVIDUAL_IDENTIFIER
  </xsl:variable>

  <xsl:variable name="PROVIDER_ORGANISATION_IDENTIFIER_CSS_CLASS">PROVIDER_ORGANISATION_IDENTIFIER
  </xsl:variable>

  <xsl:variable name="PROVIDER_DEVICE_IDENTIFIER_CSS_CLASS">PROVIDER_DEVICE_IDENTIFIER
  </xsl:variable>

  <!-- CDA Document Type -->
  <xsl:variable name="PROVIDER_INDIVIDUAL_QUALIFICATIONS_CSS_CLASS">
    PROVIDER_INDIVIDUAL_QUALIFICATIONS
  </xsl:variable>

  <!-- CDA Document Title -->
  <xsl:variable name="PROVIDER_INDIVIDUAL_OCCUPATION_CSS_CLASS">PROVIDER_INDIVIDUAL_OCCUPATION
  </xsl:variable>

  <xsl:variable name="DH_GENERIC_CDA_STYLESHEET_SUPPORTED_CDA_RENDERING_SPECIFICATION_VERSION">1.0
  </xsl:variable>

  <!-- CDA Document Creation Date Time -->
  <xsl:variable
    name="DH_GENERIC_CDA_STYLESHEET_SUPPORTED_CDA_RENDERING_SPECIFICATION_MAJOR_VERSION"
    select="substring-before($DH_GENERIC_CDA_STYLESHEET_SUPPORTED_CDA_RENDERING_SPECIFICATION_VERSION, '.')"/>

  <!-- CDA Document Creation Date  -->
  <xsl:variable
    name="DH_GENERIC_CDA_STYLESHEET_SUPPORTED_CDA_RENDERING_SPECIFICATION_MINOR_VERSION"
    select="substring-after($DH_GENERIC_CDA_STYLESHEET_SUPPORTED_CDA_RENDERING_SPECIFICATION_VERSION, '.')"/>

  <!-- CDA Document Creation Date for top of document -->
  <xsl:variable name="DH_DISCHARGE_SUMMARY_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.4
  </xsl:variable>

  <!-- Document Rendering View Title -->
  <xsl:variable name="DH_SHARED_HEALTH_SUMMARY_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.120
  </xsl:variable>

  <!-- Encounter -->
  <xsl:variable name="DH_E_REFERRAL_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.2
  </xsl:variable>

  <xsl:variable name="DH_SPECIALIST_LETTER_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.132
  </xsl:variable>

  <xsl:variable name="DH_EVENT_SUMMARY_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.136
  </xsl:variable>

  <xsl:variable name="DH_SERVICE_REFERRAL_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.100.1002.231
  </xsl:variable>

  <xsl:variable name="DH_E_PRESCRIPTION_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.74
  </xsl:variable>

  <xsl:variable name="DH_DISPENSE_RECORD_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.75
  </xsl:variable>

  <xsl:variable name="DH_PRESCRIPTION_REQUEST_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.101
  </xsl:variable>

  <xsl:variable
    name="DH_AUSTRALIAN_CHILDHOOD_IMMUNISATION_REGISTER_RECORD_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.144
  </xsl:variable>
  ...
  <!-- ENTRY TEMPLATE -->
  <!--================================================================================================================================================================================-->
  <xsl:variable name="DH_AUSTRALIAN_IMMUNISATION_REGISTER_RECORD_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.234
  </xsl:variable>
  <!--================================================================================================================================================================================-->
  <!-- [END] ENTRY TEMPLATE -->
  ...
  <xsl:variable name="DH_AUSTRALIAN_ORGAN_DONOR_REGISTER_RECORD_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.147
  </xsl:variable>
  ...
  <xsl:variable name="DH_MEDICARE_DVA_BENEFITS_REPORT_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.140
  </xsl:variable>

  <xsl:variable name="DH_ADVANCE_CARE_DIRECTIVE_CUSTODIAN_RECORD_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.156
  </xsl:variable>
  ...
  <xsl:variable name="DH_PHARMACEUTICAL_BENEFITS_REPORT_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.142
  </xsl:variable>

  <xsl:variable name="DH_CONSUMER_ENTERED_NOTES_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.151
  </xsl:variable>

  <xsl:variable name="DH_CONSUMER_ENTERED_HEALTH_SUMMARY_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.153
  </xsl:variable>

  <xsl:variable name="DH_CONSOLIDATED_VIEW_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.101.100.1002.160
  </xsl:variable>

  <xsl:variable name="DH_MEDICARE_INFORMATION_VIEW_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.100.1002.172
  </xsl:variable>

  <xsl:variable name="DH_MHR_PRESCRIPTION_RECORD_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.100.1002.170
  </xsl:variable>

  <xsl:variable name="DH_MHR_DISPENSE_RECORD_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.100.1002.171
  </xsl:variable>

  <xsl:variable name="DH_MHR_PRESCRIPTION_AND_DISPENSE_VIEW_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.100.1002.179
  </xsl:variable>

  <xsl:variable name="DH_PATHOLOGY_REPORT_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.100.1002.220
  </xsl:variable>

  <xsl:variable name="DH_DIAGNOSTIC_IMAGING_REPORT_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.100.1002.222
  </xsl:variable>
  ...
  <!-- DETAILS TEMPLATES -->
  <!--================================================================================================================================================================================-->
  <xsl:variable name="DH_ADVANCE_CARE_INFORMATION_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.100.1002.226
  </xsl:variable>

  <xsl:variable name="DH_PHARMACIST_SHARED_MEDICINES_LIST_CDA_IMPLEMENTATION_GUIDE_OID">
    1.2.36.1.2001.1001.100.1002.237
  </xsl:variable>

  <xsl:variable name="DH_DISCHARGE_SUMMARY_CLINICAL_DOCUMENT_TYPE_NAME">Discharge Summary
  </xsl:variable>

  <xsl:variable name="DH_SHARED_HEALTH_SUMMARY_CLINICAL_DOCUMENT_TYPE_NAME">Shared Health Summary
  </xsl:variable>
  ...
  <xsl:variable name="DH_SPECIALIST_LETTER_CLINICAL_DOCUMENT_TYPE_NAME">Specialist Letter
  </xsl:variable>
  ...
  <xsl:variable name="DH_SERVICE_REFERRAL_CLINICAL_DOCUMENT_TYPE_NAME">Service Referral
  </xsl:variable>

  <xsl:variable name="DH_E_PRESCRIPTION_CLINICAL_DOCUMENT_TYPE_NAME">e-Prescription</xsl:variable>

  <xsl:variable name="DH_DISPENSE_RECORD_CLINICAL_DOCUMENT_TYPE_NAME">Dispense Record
  </xsl:variable>

  <xsl:variable name="DH_PRESCRIPTION_REQUEST_CLINICAL_DOCUMENT_TYPE_NAME">Prescription Request
  </xsl:variable>
  ...
  <xsl:variable
    name="DH_AUSTRALIAN_CHILDHOOD_IMMUNISATION_REGISTER_RECORD_CLINICAL_DOCUMENT_TYPE_NAME">
    Australian Childhood Immunisation Register
  </xsl:variable>

  <xsl:variable name="DH_AUSTRALIAN_IMMUNISATION_REGISTER_RECORD_CLINICAL_DOCUMENT_TYPE_NAME">
    Australian Immunisation Register
  </xsl:variable>

  <xsl:variable name="DH_AUSTRALIAN_ORGAN_DONOR_REGISTER_RECORD_CLINICAL_DOCUMENT_TYPE_NAME">
    Australian Organ Donor Register Record
  </xsl:variable>

  <xsl:variable name="DH_MEDICARE_DVA_BENEFITS_REPORT_CLINICAL_DOCUMENT_TYPE_NAME">Medicare/DVA
    Benefits Report
  </xsl:variable>

  <xsl:variable name="DH_ADVANCE_CARE_DIRECTIVE_CUSTODIAN_RECORD_CLINICAL_DOCUMENT_TYPE_NAME">
    Advance Care Directive Custodian Record
  </xsl:variable>

  <xsl:variable name="DH_PHARMACEUTICAL_BENEFITS_REPORT_CLINICAL_DOCUMENT_TYPE_NAME">
    Pharmaceutical
    Benefits Report
  </xsl:variable>

  <xsl:variable name="DH_CONSUMER_ENTERED_NOTES_CLINICAL_DOCUMENT_TYPE_NAME">Personal Health Note
  </xsl:variable>

  <xsl:variable name="DH_CONSUMER_ENTERED_HEALTH_SUMMARY_CLINICAL_DOCUMENT_TYPE_NAME">Personal
    Health Summary
  </xsl:variable>

  <xsl:variable name="DH_CONSOLIDATED_VIEW_CLINICAL_DOCUMENT_TYPE_NAME">Consolidated View
  </xsl:variable>

  <xsl:variable name="DH_MEDICARE_INFORMATION_VIEW_CLINICAL_DOCUMENT_TYPE_NAME">Medicare Overview
  </xsl:variable>

  <xsl:variable name="DH_MHR_PRESCRIPTION_RECORD_CLINICAL_DOCUMENT_TYPE_NAME">eHealth Prescription
    Record
  </xsl:variable>

  <xsl:variable name="DH_MHR_DISPENSE_RECORD_CLINICAL_DOCUMENT_TYPE_NAME">eHealth Dispense Record
  </xsl:variable>

  <xsl:variable name="DH_MHR_PRESCRIPTION_AND_DISPENSE_VIEW_CLINICAL_DOCUMENT_TYPE_NAME">
    Prescription and Dispense View
  </xsl:variable>

  <xsl:variable name="DH_PATHOLOGY_REPORT_CLINICAL_DOCUMENT_TYPE_NAME">Pathology Report
  </xsl:variable>

  <xsl:variable name="DH_DIAGNOSTIC_IMAGING_REPORT_CLINICAL_DOCUMENT_TYPE_NAME">Diagnostic Imaging
    Report
  </xsl:variable>

  <xsl:variable name="DH_ADVANCE_CARE_INFORMATION_CLINICAL_DOCUMENT_TYPE_NAME">Advance Care
    Information
  </xsl:variable>

  <xsl:variable name="DH_PHARMACIST_SHARED_MEDICINES_LIST_CLINICAL_DOCUMENT_TYPE_NAME">Pharmacist
  ...
  <xsl:variable name="THREE_QUARTER_CONTENT_WIDTH_PX"
    select="($FULL_CONTENT_WIDTH_PX div 4) * 3"/>
  ...
  <xsl:variable name="cdaDocumentCdaRenderingSpecificationVersion"
    select="/cda:ClinicalDocument/cda:templateId[@root='1.2.36.1.2001.1001.100.149']/@extension"/>

  <xsl:variable name="cdaDocumentCdaRenderingSpecificationMajorVersionNumber"
    select="substring-before($cdaDocumentCdaRenderingSpecificationVersion, '.')"></xsl:variable>

  <xsl:variable name="cdaDocumentCdaRenderingSpecificationMinorVersionNumber"
    select="substring-after($cdaDocumentCdaRenderingSpecificationVersion, '.')"></xsl:variable>
        ...
        <xsl:choose>
          <xsl:otherwise>
            <xsl:choose>
              <xsl:otherwise>true</xsl:otherwise>
              <xsl:when
                test="$cdaDocumentCdaRenderingSpecificationMajorVersionNumber = $DH_GENERIC_CDA_STYLESHEET_SUPPORTED_CDA_RENDERING_SPECIFICATION_MAJOR_VERSION">
                <xsl:choose>
                  <xsl:otherwise>true</xsl:otherwise>
                  <xsl:when
                    test="$cdaDocumentCdaRenderingSpecificationMinorVersionNumber &gt; $DH_GENERIC_CDA_STYLESHEET_SUPPORTED_CDA_RENDERING_SPECIFICATION_MINOR_VERSION">
                    false
                  </xsl:when>
                </xsl:choose>
              </xsl:when>
  ...
  <xsl:variable name="patientIHI">
    <xsl:call-template name="trim">
      <xsl:with-param name="string">
        <xsl:call-template name="formatHI">
          <xsl:with-param name="hiValue"
            select="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/ext:asEntityIdentifier/ext:id[@assigningAuthorityName='IHI' or substring(@root,1,27)='1.2.36.1.2001.1003.0.800360']/@root"/>
        </xsl:call-template>
      </xsl:with-param>
    </xsl:call-template>
  </xsl:variable>
  ...
  <xsl:variable name="patientMRN">
    <xsl:if test="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/ext:asEntityIdentifier[ext:code/@code='MR']/ext:id/@extension and
      ...
      <xsl:value-of
        select="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/ext:asEntityIdentifier[ext:code/@code='MR']/ext:id/@extension"/>
    </xsl:if>
  </xsl:variable>
  ...
  <xsl:variable name="patientFamilyName">
    <xsl:call-template name="toUpperCase">
      <xsl:with-param name="data"
        select="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/cda:name[1]/cda:family"/>
    </xsl:call-template>
  </xsl:variable>

  <xsl:variable name="patientGivenName"
    select="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/cda:name[1]/cda:given"/>

  <xsl:variable name="patientPrefix"
    select="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/cda:name[1]/cda:prefix"/>

  <xsl:variable name="patientFamilyNameNullFlavor">
    <xsl:if
      test="string-length(/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/cda:name[1]/cda:family/@nullFlavor) &gt; 0">
      <xsl:call-template name="show-null-flavor-value">
        <xsl:with-param name="nullFlavorValue">
          <xsl:value-of
            select="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/cda:name[1]/cda:family/@nullFlavor"/>
        </xsl:with-param>
      </xsl:call-template>
    </xsl:if>
  </xsl:variable>
  ...
  <xsl:variable name="patientDeceasedStatus">
    <xsl:choose>
      ...
      <xsl:when test="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/ext:deceasedInd and
                ...
                /cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/ext:deceasedInd/@value='true'">
        true
      </xsl:when>
    </xsl:choose>
  </xsl:variable>
    ...
    <xsl:call-template name="getCodedAge">
      <xsl:with-param name="ageValue">
        <xsl:choose>
          <xsl:otherwise/>
          <xsl:when
            test="string-length(/cda:ClinicalDocument/cda:component/cda:structuredBody/cda:component/cda:section/cda:entry/cda:observation[cda:code/@code='103.20109' and cda:code/@codeSystem='1.2.36.1.2001.1001.101']/cda:value/@value) &gt; 0">
            <xsl:value-of
              select="/cda:ClinicalDocument/cda:component/cda:structuredBody/cda:component/cda:section/cda:entry/cda:observation[cda:code/@code='103.20109' and cda:code/@codeSystem='1.2.36.1.2001.1001.101']/cda:value/@value"/>
          </xsl:when>
        </xsl:choose>
      </xsl:with-param>
      <xsl:with-param name="ageUcumUnitCode">
        <xsl:choose>
          <xsl:otherwise/>
          <xsl:when
            test="string-length(/cda:ClinicalDocument/cda:component/cda:structuredBody/cda:component/cda:section/cda:entry/cda:observation[cda:code/@code='103.20109' and cda:code/@codeSystem='1.2.36.1.2001.1001.101']/cda:value/@unit) &gt; 0">
            <xsl:value-of
              select="/cda:ClinicalDocument/cda:component/cda:structuredBody/cda:component/cda:section/cda:entry/cda:observation[cda:code/@code='103.20109' and cda:code/@codeSystem='1.2.36.1.2001.1001.101']/cda:value/@unit"/>
          </xsl:when>
        </xsl:choose>
      </xsl:with-param>
    </xsl:call-template>
  ...
  <xsl:variable name="patientAge">
    <xsl:choose>
      ...
      <xsl:when
        test="((string-length(/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/cda:birthTime/@value) &gt; 7) and
        ...
        <xsl:call-template name="calculateAge">
          ...
          <xsl:with-param name="docDate">
            <xsl:choose>
              <xsl:otherwise>
                <xsl:value-of
                  select="substring(/cda:ClinicalDocument/cda:effectiveTime/@value,1,8)"/>
              </xsl:otherwise>
              <xsl:when
                test="string-length(/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:effectiveTime/cda:high/@value) &gt; 0">
                <xsl:value-of
                  select="substring(/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:effectiveTime/cda:high/@value,1,8)"/>
              </xsl:when>
            </xsl:choose>
          </xsl:with-param>
        </xsl:call-template>
  ...
  <xsl:variable name="patientSex">
    <xsl:if test="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/cda:administrativeGenderCode and
      ...
      <xsl:choose>
        ...
        <xsl:when
          test="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/cda:administrativeGenderCode/@code = 'I'">
          Intersex or Indeterminate
        </xsl:when>
        <xsl:when
          test="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/cda:administrativeGenderCode/@code = 'N'">
  ...
  <xsl:variable name="patientIndigenousStatus">
    <xsl:if test="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/cda:ethnicGroupCode/@code and
      ...
      <xsl:value-of
        select="/cda:ClinicalDocument/cda:recordTarget/cda:patientRole/cda:patient/cda:ethnicGroupCode/@displayName"/>
    </xsl:if>
  </xsl:variable>
  ...
  <xsl:variable name="dischargeLocation">
    <xsl:if test="/cda:ClinicalDocument/cda:participant[@typeCode='ORG' and cda:associatedEntity/@classCode='SDLOC']/cda:associatedEntity/cda:code/cda:originalText and
      ...
      <xsl:value-of
        select="/cda:ClinicalDocument/cda:participant[@typeCode='ORG' and cda:associatedEntity/@classCode='SDLOC']/cda:associatedEntity/cda:code/cda:originalText"/>
    </xsl:if>
  </xsl:variable>

  <xsl:variable name="separationMode"
    select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:dischargeDispositionCode/@displayName"/>
  ...
  <xsl:variable name="lengthOfStay">
    <xsl:if test="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:effectiveTime/cda:low and
      ...
      <xsl:variable name="date1"
        select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:effectiveTime/cda:low/@value"/>
    </xsl:if>
  </xsl:variable>
  ...
  <xsl:variable name="cdaDocumentType">
    <xsl:choose>
      ...
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_DISCHARGE_SUMMARY_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_DISCHARGE_SUMMARY_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_SHARED_HEALTH_SUMMARY_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_SHARED_HEALTH_SUMMARY_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_E_REFERRAL_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_E_REFERRAL_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_SPECIALIST_LETTER_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_SPECIALIST_LETTER_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_EVENT_SUMMARY_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_EVENT_SUMMARY_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_SERVICE_REFERRAL_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_SERVICE_REFERRAL_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_E_PRESCRIPTION_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_E_PRESCRIPTION_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_DISPENSE_RECORD_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_DISPENSE_RECORD_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_PRESCRIPTION_REQUEST_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_PRESCRIPTION_REQUEST_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_AUSTRALIAN_CHILDHOOD_IMMUNISATION_REGISTER_RECORD_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of
          select="$DH_AUSTRALIAN_CHILDHOOD_IMMUNISATION_REGISTER_RECORD_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_AUSTRALIAN_IMMUNISATION_REGISTER_RECORD_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of
          select="$DH_AUSTRALIAN_IMMUNISATION_REGISTER_RECORD_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_AUSTRALIAN_ORGAN_DONOR_REGISTER_RECORD_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of
          select="$DH_AUSTRALIAN_ORGAN_DONOR_REGISTER_RECORD_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_MEDICARE_DVA_BENEFITS_REPORT_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of
          select="$DH_MEDICARE_DVA_BENEFITS_REPORT_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_PHARMACEUTICAL_BENEFITS_REPORT_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of
          select="$DH_PHARMACEUTICAL_BENEFITS_REPORT_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_ADVANCE_CARE_DIRECTIVE_CUSTODIAN_RECORD_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of
          select="$DH_ADVANCE_CARE_DIRECTIVE_CUSTODIAN_RECORD_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_CONSUMER_ENTERED_NOTES_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_CONSUMER_ENTERED_NOTES_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_CONSUMER_ENTERED_HEALTH_SUMMARY_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of
          select="$DH_CONSUMER_ENTERED_HEALTH_SUMMARY_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_CONSOLIDATED_VIEW_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_CONSOLIDATED_VIEW_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_MEDICARE_INFORMATION_VIEW_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_MEDICARE_INFORMATION_VIEW_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_MHR_PRESCRIPTION_RECORD_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_MHR_PRESCRIPTION_RECORD_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_MHR_DISPENSE_RECORD_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_MHR_DISPENSE_RECORD_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_MHR_PRESCRIPTION_AND_DISPENSE_VIEW_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of
          select="$DH_MHR_PRESCRIPTION_AND_DISPENSE_VIEW_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_PATHOLOGY_REPORT_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_PATHOLOGY_REPORT_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_DIAGNOSTIC_IMAGING_REPORT_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_DIAGNOSTIC_IMAGING_REPORT_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_ADVANCE_CARE_INFORMATION_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of select="$DH_ADVANCE_CARE_INFORMATION_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
      <xsl:when
        test="/cda:ClinicalDocument/cda:templateId[@root=$DH_PHARMACIST_SHARED_MEDICINES_LIST_CDA_IMPLEMENTATION_GUIDE_OID]">
        <xsl:value-of
          select="$DH_PHARMACIST_SHARED_MEDICINES_LIST_CLINICAL_DOCUMENT_TYPE_NAME"/>
      </xsl:when>
  ...
  <xsl:variable name="BANNER_HEIGHT_PX"
    select="90 + (ceiling(string-length($cdaDocumentTitle) div 90) * 20)"/>
    ...
    <xsl:call-template name="trim">
      <xsl:with-param name="string">
        <xsl:choose>
          ...
          <xsl:when
            test="string-length($patientMRN) &gt; 0 and string-length($patientIHI) &gt; 0">
            <xsl:value-of select="concat($patientName,
                                                     ' | ',
                                                     $patientSex,
                                                     ' | DoB: ',
  ...
  <xsl:variable name="encounterEffectiveTimeNullFlavor">
    <xsl:call-template name="show-null-flavor-value">
      <xsl:with-param name="nullFlavorValue"
        select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:effectiveTime/@nullFlavor"></xsl:with-param>
    </xsl:call-template>
  </xsl:variable>
    ...
    <xsl:if test="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:effectiveTime/cda:width/@value!='' and
      ...
      <xsl:call-template name="getDuration">
        <xsl:with-param name="value"
          select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:effectiveTime/cda:width/@value"/>
        <xsl:with-param name="ucumUnitCode"
          select="/cda:ClinicalDocument/cda:componentOf/cda:encompassingEncounter/cda:effectiveTime/cda:width/@unit"/>
      </xsl:call-template>
    </xsl:if>
  </xsl:variable>
